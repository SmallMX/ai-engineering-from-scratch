# 音频 评估：WER, MOS, UTMOS, MMAU, FAD,与the Open Leaderboards

> You cannot ship what you cannot measure. This lesson names the 2026 metrics for every audio task：ASR (WER, CER, RTFx), TTS (MOS, UTMOS, SECS, WER-on-ASR-round-trip), audio-language (MMAU, LongAudioBench), music (FAD, CLAP),与speaker (EER). Plus the leaderboards where you compare.

**类型：** 学习
**语言：** Python
**前置知识：** Phase 6 · 04, 06, 07, 09, 10; Phase 2 · 09 (Model 评估)
**时间：** 约 60 minutes

## 学习目标
- 理解 音频 评估：WER, MOS, UTMOS, MMAU, FAD,与the Open Leaderboards 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 6「语音与音频」的第 17 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
