# 声谱图, Mel Scale与音频 Features

> Neural nets do not consume raw waveforms well. They consume spectrograms. They consume mel spectrograms even better. Every ASR, TTS,与audio classifier in 2026 lives or dies by this single preprocessing choice.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 6 · 01 (音频 基础)
**时间：** 约 45 minutes

## 学习目标
- 理解 声谱图, Mel Scale与音频 Features 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 6「语音与音频」的第 02 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
