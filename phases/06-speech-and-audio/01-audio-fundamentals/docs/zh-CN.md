# 音频 基础：Waveforms, Sampling, Fourier Transform

> Waveforms are the raw signal. 声谱图 are the representation. Mel features are the ML-friendly form. Every modern ASR与TTS pipeline walks this ladder,与the first rung is understanding sampling与Fourier.

**类型：** 学习
**语言：** Python
**前置知识：** Phase 1 · 06 (Vectors与Matrices), Phase 1 · 14 (Probability Distributions)
**时间：** 约 45 minutes

## 学习目标
- 理解 音频 基础：Waveforms, Sampling, Fourier Transform 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 6「语音与音频」的第 01 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
