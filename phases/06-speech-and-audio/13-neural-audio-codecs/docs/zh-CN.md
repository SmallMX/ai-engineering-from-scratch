# Neural 音频 Codecs：EnCodec, SNAC, Mimi, DAC与the Semantic-Acoustic Split

> 2026 audio generation is almost all tokens. EnCodec, SNAC, Mimi,与DAC turn continuous waveforms into discrete sequences that a transformer can predict. The semantic-vs-acoustic token split：first-codebook as semantic, rest as acoustic：is the most important architectural shift since the Transformer for audio.

**类型：** 学习
**语言：** Python
**前置知识：** Phase 6 · 02 (声谱图), Phase 10 · 11 (量化), Phase 5 · 19 (Subword 分词)
**时间：** 约 60 minutes

## 学习目标
- 理解 Neural 音频 Codecs：EnCodec, SNAC, Mimi, DAC与the Semantic-Acoustic Split 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 6「语音与音频」的第 13 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
