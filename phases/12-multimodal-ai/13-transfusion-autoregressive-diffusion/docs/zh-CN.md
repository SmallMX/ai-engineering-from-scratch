# Transfusion：Autoregressive Text + 扩散 图像 in One Transformer

> Chameleon与Emu3 bet everything on discrete tokens. They work, but the quantization bottleneck is visible：the image quality plateaus below continuous-space diffusion models. Transfusion (Meta, Zhou et al., August 2024) takes the opposite bet：keep images continuous, drop the VQ-VAE entirely,与train one transformer with two losses. Text tokens get next-token-prediction. 图像 patches get a flow-matching / diffusion loss. Both objectives optimize the same weights. The architecture underlying Stable 扩散 3 (MMDiT) is a close cousin. This lesson reads the Transfusion thesis, builds a toy two-loss trainer,与traces the attention mask that lets one transformer do both jobs.
...
- Compare Transfusion-style (continuous images, diffusion loss) to Chameleon-style (discrete images, NTP) on compute, quality,与code complexity.
- Name MMDiT's contribution：modality-specific weights at each block, joint attention at the residual stream.

## 中文导读

本课是 Phase 12「多模态 AI」的第 13 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
