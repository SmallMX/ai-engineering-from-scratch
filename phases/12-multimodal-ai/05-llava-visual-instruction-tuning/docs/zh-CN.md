# LLaVA与Visual Instruction Tuning

> LLaVA (April 2023) is the most copied multimodal architecture on the planet. It replaced BLIP-2's Q-Former with a 2-layer MLP, replaced Flamingo's gated cross-attention with naive token concatenation,与trained on 158k visual-instruction turns generated by GPT-4 from text-only captions. Any practitioner who built a VLM between 2023与2026 built some variant of LLaVA. LLaVA-1.5 added AnyRes. LLaVA-NeXT bumped resolution. LLaVA-OneVision unified image, multi-image,与video in one recipe. This lesson reads the recipe, implements the projector,与explains why "simpler won."

**类型：** 构建
**语言：** Python (stdlib, projector + instruction-template builder)
**前置知识：** Phase 12 · 02 (CLIP), Phase 11 (LLM 工程：instruction tuning)
**时间：** 约 180 minutes
...
- Construct a LLaVA-format prompt with the image token placeholder, system prompt,与user/assistant turns.
- Explain why the community moved from Q-Former to MLP despite Q-Former's token-budget win.

## 中文导读

本课是 Phase 12「多模态 AI」的第 05 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
