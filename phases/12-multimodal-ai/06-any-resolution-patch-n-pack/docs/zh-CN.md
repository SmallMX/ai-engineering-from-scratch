...
## 学习目标
- Pack patches from a batch of variable-resolution images into one sequence与build the block-diagonal attention mask.
- Pick between AnyRes tiling (LLaVA-NeXT), NaFlex (SigLIP 2),与M-RoPE (Qwen2-VL) for a given task.
- Compute token budgets for OCR, charts,与photography without resizing.
- Name the three failure modes of square-resize：squished text, cropped content, wasted tokens on padding.

## 中文导读

本课是 Phase 12「多模态 AI」的第 06 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
