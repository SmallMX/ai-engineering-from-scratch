# Long-视频 Understanding at Million-Token Context

> A 1-hour 4K video at 24 FPS, patched与embedded, produces on the order of 60 million tokens. A 2-hour podcast episode transcribed is 30,000 tokens. A full Blu-ray feature film, even compressed with aggressive pooling, is hundreds of thousands of tokens. Google's Gemini 1.5 (March 2024) opened this era with a 10-million-token context, doing reliable needle-in-a-haystack recall over hour-long videos. LWM (Liu et al., February 2024) showed ring attention's scaling path. LongVILA与视频-XL scaled ingestion further. VideoAgent swapped raw context for agentic retrieval. Each approach is a different trade-off on compute, recall,与engineering complexity. This lesson reads them side by side.
...
## 学习目标
- Compute total visual-token counts for long-form video at varying FPS与pooling.
- Explain the three scaling paths：brute context (Gemini 1.5), ring attention (LWM), token compression (LongVILA / 视频-XL).
- Compare raw-context video VLMs vs agentic-retrieval video VLMs (VideoAgent) on accuracy与latency.
- Design a needle-in-a-haystack test for a 30-minute video与measure recall at a specific minute.

## 中文导读

本课是 Phase 12「多模态 AI」的第 18 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
