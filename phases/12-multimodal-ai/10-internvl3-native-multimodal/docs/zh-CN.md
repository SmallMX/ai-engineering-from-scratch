# InternVL3：Native 多模态 Pretraining

> Every open VLM before InternVL3 followed the same three-step recipe：take a text LLM trained on trillions of text tokens, bolt on a vision encoder, then fine-tune the seams. This works but has alignment debt：the text LLM has spent its full pretraining budget on pure text与does not natively understand visual tokens. When you add vision post-hoc, the LLM has to re-learn how to relate visual input to its text reasoning without forgetting the text. InternVL3 (Zhu et al., April 2025) rejects the post-hoc approach：one pretraining run, text与multimodal interleaved from step one. The result matches Gemini 2.5 Pro on MMMU-Pro at 78B params open. This lesson reads the case for native pretraining与what changes when you make it.
...
- Describe InternVL3's native pretraining corpus mix与why the ratio of text ：interleaved ：caption matters.
- Compare V2PE (variable visual position encoding) to Qwen2-VL's M-RoPE.
- Name the Visual Resolution Router (ViR)与Decoupled 视觉-Language (DvD) deployment optimizations.

## 中文导读

本课是 Phase 12「多模态 AI」的第 10 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
