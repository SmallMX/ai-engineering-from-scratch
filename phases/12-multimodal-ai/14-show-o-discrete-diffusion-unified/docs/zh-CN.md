# Show-o与Discrete-扩散 Unified Models

> Transfusion mixes continuous与discrete representations. Show-o (Xie et al., August 2024) goes the other way：text tokens use causal next-token prediction, image tokens use masked discrete diffusion in the spirit of MaskGIT. Both sit inside one transformer with a hybrid attention mask. The result unifies VQA, text-to-image, inpainting,与mixed-modality generation on one backbone, one tokenizer per modality, one loss formulation (next-token extended to masked prediction). This lesson walks the Show-o design：why masked discrete diffusion is a parallel, few-step image generator —与contrasts with Transfusion与Emu3.
...
- Compare parallel image decoding (Show-o, MaskGIT) to autoregressive image decoding (Chameleon, Emu3) on speed与quality.
- Name the three tasks Show-o handles in one checkpoint：T2I, VQA, image inpainting.
- Pick a masking schedule (cosine, linear, truncated)与reason about its effect on sample quality.

## 中文导读

本课是 Phase 12「多模态 AI」的第 14 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
