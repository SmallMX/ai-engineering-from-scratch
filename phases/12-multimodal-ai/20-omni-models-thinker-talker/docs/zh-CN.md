# Omni Models：Qwen2.5-Omni与the Thinker-Talker Split

> GPT-4o's product demo in May 2024 was disruptive not because of the underlying model but because of the product shape：a voice interface where you talk, the model sees what the camera sees,与it talks back in under 250ms. The open ecosystem spent the rest of 2024与2025 racing to reach that product surface. Qwen2.5-Omni (March 2025) is the reference open design：a Thinker (large text-generating transformer) plus a Talker (parallel speech-generating transformer), linked by streaming speech tokens. Mini-Omni simplified it, Moshi matched its latency, GLM-4-语音 extended it to Chinese. This lesson reads the Thinker-Talker architecture与the latency budget that makes streaming real-time dialogue work.

**类型：** 构建
**语言：** Python (stdlib, streaming pipeline latency simulator + VAD loop)
**前置知识：** Phase 12 · 19 (audio-LLM), Phase 12 · 16 (any-to-any)
**时间：** 约 180 minutes

## 学习目标
- Split the inference pipeline into Thinker (text reasoning)与Talker (speech synthesis)与explain why parallel streaming works.
- Compute the time-to-first-audio-byte (TTFAB) budget for a conversational interaction, component by component.
- Describe TMRoPE's time-aligned position encoding across vision, audio,与text within the Thinker.
- Name the three real-time conversational patterns：half-duplex, turn-taking, full-duplex.

## 中文导读

本课是 Phase 12「多模态 AI」的第 20 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
