...
> CLIP aligns image与text but cannot generate captions, answer questions, or hold a conversation. BLIP-2 (Salesforce, 2023) solved that with a small trainable bridge：32 learnable query vectors attend over a frozen ViT's features via cross-attention, then slot directly into a frozen LLM's input stream. 188M parameters of bridge connected an 11B LLM to a ViT-g/14. Every adapter-based VLM through 2026：MiniGPT-4, InstructBLIP, LLaVA's cousins：is a descendant. This lesson reads the Q-Former's architecture, explains its two-stage training,与builds a toy version that feeds visual tokens into a frozen text decoder.
...
## 学习目标
- Explain why a trainable bottleneck between a frozen vision encoder与frozen LLM beats end-to-end finetuning in cost与stability.
...
- Compare Q-Former to the simpler MLP projector used in LLaVA与argue when each choice wins.

## 中文导读

本课是 Phase 12「多模态 AI」的第 03 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
