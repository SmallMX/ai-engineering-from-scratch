...
> The first time a model read a recipe off a website与executed it in a kitchen robot was RT-2 (Google DeepMind, July 2023). RT-2 discretized actions as text tokens, co-fine-tuned a VLM on web data plus robot-action data,与proved that web-scale vision-language knowledge transfers to robotic control. OpenVLA (June 2024) shipped the open 7B reference. Physical Intelligence's π0 series (2024-2025) added flow-matching action experts. NVIDIA's GR00T N1 (March 2025) delivered dual-system (系统 1 / 系统 2) control for humanoid robots at scale. The VLA primitive：vision-language-action, a single model that sees, reads,与acts：is the bridge between this phase's understanding models与the autonomous systems in Phase 15.

**类型：** 学习
**语言：** Python (stdlib, action tokenizer + VLA inference skeleton)
**前置知识：** Phase 12 · 05 (LLaVA), Phase 15 (自主 系统, referenced)
**时间：** 约 180 minutes
...
- Compare OpenVLA (open 7B Llama+VLM), π0 (flow-matching),与GR00T N1 (dual-system) on the same robot task.
- Name the Open X-Embodiment dataset与its role as the RT-X training corpus.

## 中文导读

本课是 Phase 12「多模态 AI」的第 21 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
