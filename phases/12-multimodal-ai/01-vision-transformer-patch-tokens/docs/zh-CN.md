# 视觉 Transformer与the Patch-Token Primitive

> Before anything multimodal, an image has to become a sequence of tokens a transformer can eat. The 2020 ViT paper answered this with 16x16 pixel patches, a linear projection,与a position embedding. Five years later every 2026 frontier model (Claude Opus 4.7 at 2576px native, Gemini 3.1 Pro, Qwen3.5-Omni) still begins this way：the encoder changed from ViT to DINOv2 to SigLIP 2, register tokens were added, the positional scheme became 2D-RoPE, but the primitive held. This lesson reads the patch-token pipeline end to end与builds it in stdlib Python so the rest of Phase 12 has a concrete mental model for "visual tokens."

**类型：** 学习
**语言：** Python (stdlib, patch tokenizer + geometry calculator)
**前置知识：** Phase 7 (Transformer), Phase 4 (Computer 视觉)
**时间：** 约 120 minutes

## 学习目标
- Convert an HxWx3 image into a sequence of patch tokens with correct positional encoding.
- Compute sequence length, parameter count,与FLOPs for a ViT of a given (patch size, resolution, hidden dim, depth).
- Name the three upgrades that took ViT from 2020 research to 2026 production：self-supervised pretraining (DINO / MAE), register tokens,与native-resolution packing.
- Pick between CLS pooling, mean pooling,与register tokens for a downstream task.

## 中文导读

本课是 Phase 12「多模态 AI」的第 01 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
