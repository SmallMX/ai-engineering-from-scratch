# CLIP与Contrastive 视觉-Language Pretraining

> OpenAI's CLIP (2021) proved a single idea big enough to power the next five years：align an image encoder与a text encoder in the same vector space using only noisy web image-caption pairs与a contrastive loss. Zero supervised labels. 400M pairs. The resulting embedding space does zero-shot classification, image-text retrieval,与plugs into every 2026 VLM as its vision tower. SigLIP 2 (2025) replaced softmax with sigmoid与scaled past CLIP at lower cost. This lesson walks the math from InfoNCE to sigmoid pairwise loss与builds the training step in stdlib Python.

**类型：** 构建
**语言：** Python (stdlib, InfoNCE + sigmoid loss implementations)
**前置知识：** Phase 12 · 01 (ViT patches), Phase 7 (Transformer)
**时间：** 约 180 minutes

## 学习目标
- Derive InfoNCE loss from mutual information与implement a numerically-stable vectorized version.
- Explain why sigmoid pairwise loss (SigLIP) scales to batch 32768+ without the all-gather overhead softmax demands.
- Run zero-shot ImageNet classification by constructing text templates (`a photo of a {class}`)与taking argmax over cosine similarity.
- Name the four levers CLIP / SigLIP pretraining gives you：batch size, temperature, prompt template, data quality.

## 中文导读

本课是 Phase 12「多模态 AI」的第 02 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
