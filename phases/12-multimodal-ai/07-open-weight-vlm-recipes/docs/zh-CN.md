...
- Read an MM1 / Idefics2 / Cambrian-1 ablation table与predict which knob moves a given benchmark.
- Pick a recipe (encoder, connector, data, resolution) for a new VLM given a compute budget与task mix.
- Explain why detailed human captions beat GPT-4V distillation at the same token count.

## 中文导读

本课是 Phase 12「多模态 AI」的第 07 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
