# Chameleon与Early-Fusion Token-Only 多模态 Models

> Every VLM we have seen so far keeps images与text separate. Visual tokens come from a vision encoder, flow into a projector, then meet text inside the LLM. The vision与text vocabularies never overlap. Chameleon (Meta, May 2024) asked：what if they did? Train a VQ-VAE that turns an image into a sequence of discrete tokens from a shared vocabulary. Every multimodal document is now one sequence：text tokens与image tokens interleaved, a single autoregressive loss. Side effect：the model can generate mixed-modality outputs：alternating text与image tokens in a single inference call. This lesson reads the early-fusion thesis与builds a toy version end to end.
...
- Compare Chameleon vs BLIP-2's Q-Former approach与describe when each is the right choice.

## 中文导读

本课是 Phase 12「多模态 AI」的第 11 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
