...
## 学习目标
- Compute M-RoPE's three-axis rotations (temporal, height, width)与explain why all three are needed.
- Pick a dynamic-FPS sampling strategy for a video与reason about tokens-per-second vs event-detection accuracy.
- Name the four Qwen-VL generational upgrades in order与what each enabled.
- Wire a Qwen2.5-VL-style JSON agent output format与parse structured tool calls from a VLM response.

## 中文导读

本课是 Phase 12「多模态 AI」的第 09 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
