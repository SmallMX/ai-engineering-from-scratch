# 多模态 智能体与Computer-Use (毕业项目)

> The 2026 frontier product is a multimodal agent that reads screenshots, clicks buttons, navigates web UIs, fills forms,与completes workflows end-to-end. SeeClick与CogAgent (2024) proved the GUI-grounding primitive. Ferret-UI added mobile. ChartAgent introduced visual tool-use for charts. VisualWebArena与AgentVista (2026) are the benchmarks the frontier chases —与even Gemini 3 Pro与Claude Opus 4.7 score ~30% on AgentVista's hard tasks. This capstone pulls together every thread of Phase 12：perception (high-res VLM), reasoning (LLM with tool use), grounding (coordinate output), long-horizon memory,与evaluation.

**类型：** 毕业项目
**语言：** Python (stdlib, action schema + agent loop skeleton)
**前置知识：** Phase 12 · 05 (LLaVA), Phase 12 · 09 (Qwen-VL JSON), Phase 14 (智能体 工程)
**时间：** 约 240 minutes
...
本课是 Phase 12「多模态 AI」的第 25 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
