# Flamingo与Gated Cross-注意力 for Few-Shot VLMs

> DeepMind's Flamingo (2022) did two things before anyone else. It showed a single model could process arbitrarily interleaved sequences of images, videos,与text.与it showed VLMs could learn in-context：give a few-shot prompt with three example (image, caption) pairs与the model captions a new image without any gradient step. The mechanism：gated cross-attention layers, inserted between the frozen LLM's existing layers, with a learned tanh gate that starts at zero so the LLM's text capability is preserved at initialization. This lesson walks Flamingo's Perceiver resampler与gated cross-attention architecture：the ancestor of Gemini's interleaved inputs与Idefics2's visual tokens.
...
本课是 Phase 12「多模态 AI」的第 04 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
