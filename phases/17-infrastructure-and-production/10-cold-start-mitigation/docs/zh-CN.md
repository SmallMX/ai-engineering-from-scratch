# Cold Start Mitigation for Serverless LLM

> A 20 GB model image takes 5-10 minutes (7B) to 20+ minutes (70B) to go from cold to serving. In a true serverless world, that is not a warm-up：it is an outage. Mitigations operate at five layers：pre-seeded node images (Bottlerocket on AWS, dual-volume arch), model streaming (NVIDIA Run:ai Model Streamer, native in vLLM), GPU memory snapshots (Modal checkpoints, up to 10x faster restart), warm pools (`min_workers=1`), tiered loading (ServerlessLLM's NVMe→DRAM→HBM pipeline, 10-200x latency reduction),与live migration that moves input tokens (KB) rather than KV cache (GB). Modal publishes 2-4s cold starts as a floor; Baseten 5-10s default, sub-second with pre-warming. This lesson teaches you to measure, budget,与stack the five layers.

**类型：** 学习
**语言：** Python (stdlib, toy cold-start path simulator)
**前置知识：** Phase 17 · 02 (推理 Platform Economics), Phase 17 · 03 (GPU Autoscaling)
**时间：** 约 60 minutes

## 学习目标
- Enumerate the five layers of cold-start mitigation与name one tool or pattern at each layer.
...
- Explain why live migration transfers input tokens (KB) not KV cache (GB)与what the penalty is (recomputation).
- Name the warm-pool trade-off (pay for idle GPU or accept cold-start tail)与the SLA threshold at which `min_workers > 0` becomes mandatory.

## 中文导读

本课是 Phase 17「基础设施与生产部署」的第 10 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
