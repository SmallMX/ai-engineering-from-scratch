# Self-Hosted 服务部署 Selection：llama.cpp, Ollama, TGI, vLLM, SGLang

> Four engines dominate self-hosted inference in 2026. Pick based on hardware, scale,与ecosystem. **llama.cpp** is fastest on CPU：widest model support, full control over quantization与threading. **Ollama** is the dev-laptop one-command install, ~15-30% slower than llama.cpp (Go + CGo + HTTP serialization), 3x throughput gap under prod-like load. **TGI entered maintenance mode December 11, 2025**：only bug fixes, ~10% slower raw throughput than vLLM but historically top observability与HF-ecosystem integration. That maintenance status makes it a risky long-term bet：SGLang or vLLM are safer defaults for new projects. **vLLM** is the general-purpose production default：v0.15.1 (February 2026) adds PyTorch 2.10, RTX Blackwell SM120, H200 optimization. **SGLang** is the agentic multi-turn / prefix-heavy specialist：400,000+ GPUs in production (xAI, LinkedIn, Cursor, Oracle, GCP, Azure, AWS). Hardware constraints：CPU-only → llama.cpp only. AMD / non-NVIDIA → vLLM only (TRT-LLM is NVIDIA-locked). 2026 pipeline pattern：dev = Ollama, staging = llama.cpp, prod = vLLM or SGLang. Same GGUF/HF weights throughout.

**类型：** 学习
**语言：** Python (stdlib, engine-decision tree walker)
**前置知识：** All Phase 17 lessons covering engines (04, 06, 07, 09, 18)
**时间：** 约 45 minutes

## 学习目标
- Pick an engine given hardware (CPU / AMD / NVIDIA Hopper / Blackwell), scale (1 user / 100 / 10,000),与workload (general chat / agent / long-context).
- Name the 2026 TGI maintenance-mode status (December 11, 2025)与why it biases new projects toward vLLM or SGLang.
- Describe the dev/staging/prod pipeline using the same GGUF or HF weights throughout.
- Explain why "CPU only" forces llama.cpp与"AMD" excludes TRT-LLM.

## 中文导读

本课是 Phase 17「基础设施与生产部署」的第 28 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
