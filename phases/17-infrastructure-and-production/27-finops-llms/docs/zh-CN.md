...
## 学习目标
- Explain why traditional FinOps (tags + tiers) breaks on LLM spend与name the three new attribution dimensions.
- Enumerate the four token layers (prompt, tool, memory, response)与why single-bucket billing hides cost.
...
本课是 Phase 17「基础设施与生产部署」的第 27 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
