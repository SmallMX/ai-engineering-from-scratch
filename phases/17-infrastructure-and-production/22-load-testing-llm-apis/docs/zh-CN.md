...
- Design four load patterns (steady, ramp, spike, soak)与name the failure mode each catches.
- Build a realistic prompt distribution using mean + stddev of input tokens rather than fixed length.

## 中文导读

本课是 Phase 17「基础设施与生产部署」的第 22 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
