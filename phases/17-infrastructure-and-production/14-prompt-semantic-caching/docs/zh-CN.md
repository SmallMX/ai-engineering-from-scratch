# Prompt 缓存与Semantic 缓存 Economics
...
- Explain Anthropic's `cache_control` explicit marking与the two TTL options (5-min vs 1-hour) with their price multipliers.
- Compute expected monthly savings given hit rate, prompt/response mix,与token prices.
- Name the parallelization anti-pattern that inflates bills by 5-10x与the dynamic-content anti-pattern that collapses hit rate.

## 中文导读

本课是 Phase 17「基础设施与生产部署」的第 14 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
