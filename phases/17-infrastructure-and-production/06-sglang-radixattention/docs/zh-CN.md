# SGLang与RadixAttention for Prefix-Heavy Workloads
...
**前置知识：** Phase 17 · 04 (vLLM 服务部署 Internals), Phase 14 (Agentic RAG)
**时间：** 约 75 minutes

## 学习目标
- Diagram RadixAttention：how prefixes are stored in a radix tree与how KV blocks are shared across sequences rooted at the same branch.
- Explain cache-aware scheduling与why FCFS is wrong for prefix-heavy traffic.
- Compute expected speedup for a workload given prefix-cache hit rate与prompt length distribution.
- Name the prompt-ordering discipline that makes the 6.4x number real vs a lost upside.

## 中文导读

本课是 Phase 17「基础设施与生产部署」的第 06 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
