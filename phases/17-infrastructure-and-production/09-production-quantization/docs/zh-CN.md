# 生产 量化：AWQ, GPTQ, GGUF K-quants, FP8, MXFP4/NVFP4

> 量化 format is not a universal choice：it is a function of hardware, serving engine,与workload. GGUF Q4_K_M or Q5_K_M owns CPU与edge, delivered through llama.cpp与Ollama. GPTQ wins inside vLLM when you need multi-LoRA on the same base. AWQ with Marlin-AWQ kernels delivers ~741 tok/s on a 7B class model with the best Pass@1 at INT4：the 2026 default for datacenter production. FP8 stays the middle ground on Hopper, Ada,与Blackwell：near-lossless与widely supported. NVFP4与MXFP4 (Blackwell microscaling) are aggressive与require per-block validation. Two traps bite teams：calibration dataset must match deployment domain,与KV cache is separate from weight quantization：the AWQ lesson "my model is 4 GB now" forgets the 10-30 GB KV cache at production batch sizes.

**类型：** 学习
**语言：** Python (stdlib, toy memory and throughput comparison across formats)
**前置知识：** Phase 10 · 13 (量化 foundations), Phase 17 · 04 (vLLM 服务部署 Internals)
**时间：** 约 75 minutes

## 学习目标
- Name the six production quantization formats与their sweet spots in 2026.
- Pick a format given hardware (CPU vs GPU, Hopper vs Blackwell), engine (vLLM, TRT-LLM, llama.cpp),与workload (routine chat, reasoning, multi-LoRA).
- Compute the weight memory saved与the KV cache left untouched for a chosen format.
- Name the calibration-dataset pitfall that degrades quantized models on domain traffic.

## 中文导读

本课是 Phase 17「基础设施与生产部署」的第 09 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
