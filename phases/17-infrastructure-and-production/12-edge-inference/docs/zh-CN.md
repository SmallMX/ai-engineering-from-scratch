# Edge 推理：Apple Neural Engine, Qualcomm Hexagon, WebGPU/WebLLM, Jetson
...
**前置知识：** Phase 17 · 04 (vLLM 服务部署 Internals), Phase 17 · 09 (生产 量化)
**时间：** 约 60 minutes

## 学习目标
- Explain why mobile LLM inference is memory-bandwidth-bound与compute is secondary.
- Enumerate the four edge targets (Apple ANE, Qualcomm Hexagon, WebGPU/WebLLM, NVIDIA Jetson)与match each to a use case.
- Name the 2026 WebGPU coverage gap (Firefox Android catching up)与the Safari iOS 26 landing.
...
本课是 Phase 17「基础设施与生产部署」的第 12 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
