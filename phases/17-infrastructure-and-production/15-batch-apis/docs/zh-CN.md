# Batch API：the 50% Discount as Industry Standard

> Every major provider ships an async batch API with a 50% discount与~24-hour turnaround. OpenAI, Anthropic, Google,与most of the inference platforms (Fireworks batch tier, Together batch) implement the same pattern. Stack batch with prompt caching与overnight pipelines drop to ~10% of synchronous-uncached cost. The rule is brutally simple：if it is not interactive, it belongs on batch. Content generation pipelines, document classification, data extraction, report generation, bulk labeling, catalog tagging：anything tolerant of 24-hour latency is money left on the table until it moves to batch. The 2026 production pattern is to triage every new LLM workload into three lanes：interactive (synchronous with caching), semi-interactive (async queue with fallback), batch (overnight, cached input stacked). Workloads that pretend to be interactive but tolerate minutes of latency waste most.

**类型：** 学习
**语言：** Python (stdlib, toy batch-vs-sync cost simulator)
**前置知识：** Phase 17 · 14 (Prompt与Semantic 缓存)
**时间：** 约 45 minutes

## 学习目标
- Name the three provider batch API (OpenAI, Anthropic, Google)与the common 50% discount + 24h turnaround guarantees.
- Compute the cost for stacking batch + cached-input on an overnight classification workload与compare to synchronous-uncached baseline.
- Triage a workload into interactive / semi-interactive / batch与justify the lane.
- Name the two traps：partial interactivity (user expects faster than 24h)与output-schema drift (batch file format differs per provider).

## 中文导读

本课是 Phase 17「基础设施与生产部署」的第 15 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
