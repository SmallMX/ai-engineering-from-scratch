...
> A gateway sits between your apps与model providers. Core features are provider routing, fallback, retries, rate limiting, secret references, observability, guardrails. Market split in 2026：**LiteLLM** is MIT OSS with 100+ providers, OpenAI-compatible, but breaks down around ~2000 RPS (8 GB memory, cascading failures in published benchmarks); best for Python, <500 RPS, dev/prototyping. **Portkey** is control-plane-positioned (guardrails, PII redaction, jailbreak detection, audit trails), went Apache 2.0 open-source March 2026, 20-40 ms latency overhead, $49/mo production tier. **Kong AI 网关** built on Kong 网关：Kong's own benchmark on same 12 CPUs：228% faster than Portkey, 859% faster than LiteLLM; $100/model/month pricing (max 5 on Plus tier); enterprise-fit if you're already on Kong. **Bifrost** (Maxim AI)：automatic retries with configurable backoff, fallback to Anthropic on OpenAI 429. **Cloudflare / Vercel AI 网关**：managed, zero-ops, basic retry. 数据 residency drives the self-host decision; Portkey与Kong sit in the middle with OSS + optional managed.

**类型：** 学习
**语言：** Python (stdlib, toy gateway-routing simulator)
**前置知识：** Phase 17 · 01 (Managed LLM Platforms), Phase 17 · 16 (Model 路由)
**时间：** 约 60 minutes

## 学习目标
- Enumerate the six core gateway features (routing, fallback, retries, rate limits, secrets, observability, guardrails).
- Map four 2026 gateways (LiteLLM, Portkey, Kong AI, Bifrost) to scale ceilings与use cases.
- Cite the Kong benchmark (228% vs Portkey, 859% vs LiteLLM)与explain why it matters for >500 RPS.
- Choose self-hosted vs managed given data residency与ops budget.

## 中文导读

本课是 Phase 17「基础设施与生产部署」的第 19 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
