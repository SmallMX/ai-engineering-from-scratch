...
## 学习目标
- Build a custom evaluation harness that runs multiple-choice与open-ended benchmarks against a language model
- Explain why standard benchmarks (MMLU, HumanEval) saturate与fail to differentiate frontier models
- Implement task-specific evals with proper metrics：exact match, F1, BLEU,与LLM-as-judge scoring
- Design a custom evaluation suite targeting your specific use case rather than relying solely on public leaderboards

## 中文导读

本课是 Phase 10「从零构建 LLM」的第 10 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
