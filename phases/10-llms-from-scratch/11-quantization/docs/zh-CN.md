...
**前置知识：** Phase 10, Lessons 01-10 (LLM 从零实现)
**时间：** 约 120 minutes

## 学习目标
- Implement symmetric与asymmetric quantization from FP16 to INT8与INT4, including per-tensor与per-channel scaling
- Calculate the memory savings from quantization与determine which precision fits a given GPU's VRAM
- Explain the difference between post-training quantization (PTQ)与quantization-aware training (QAT)
- Apply GPTQ or AWQ to quantize a real model与measure the accuracy-memory tradeoff on a benchmark

## 中文导读

本课是 Phase 10「从零构建 LLM」的第 11 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
