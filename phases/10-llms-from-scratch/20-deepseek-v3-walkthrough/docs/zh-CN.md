...
> Phase 10 · Lesson 14 named the six architectural knobs every open model turns. DeepSeek-V3 (December 2024, 671B parameters total, 37B active) turns all six与adds four more：Multi-Head Latent 注意力, auxiliary-loss-free load balancing, Multi-Token Prediction,与DualPipe training. This lesson reads DeepSeek-V3's architecture top to bottom与derives every parameter count from the published config. By the end you can explain why the 671B/37B ratio is the right bet与why MLA + MoE together beat either alone at the frontier.

**类型：** 学习
**语言：** Python (stdlib, parameter calculator)
**前置知识：** Phase 10 · 14 (open-model walkthroughs), Phase 10 · 17 (NSA), Phase 10 · 18 (MTP), Phase 10 · 19 (DualPipe)
**时间：** 约 75 minutes

## 学习目标
- Read the DeepSeek-V3 config top to bottom与explain each field in terms of the six GPT-2 knobs plus four DeepSeek-specific additions.
- Derive the total parameter count (671B), active parameter count (37B),与the components that contribute to each.
- Compute the KV cache footprint of MLA at 128k context与compare to what a same-active-param dense model with GQA would pay.
- State the four DeepSeek-specific innovations (MLA, MTP, auxiliary-loss-free routing, DualPipe)与name which part of the architecture/training stack each one targets.

## 中文导读

本课是 Phase 10「从零构建 LLM」的第 20 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
