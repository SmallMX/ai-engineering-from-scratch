...
> DeepSeek-V3 was trained on 2,048 H800 GPUs with MoE experts scattered across nodes. Cross-node expert all-to-all communication cost 1 GPU-hour of comm for every 1 GPU-hour of compute. GPUs were idle half the time. DualPipe (DeepSeek, Dec 2024) is a bidirectional pipeline that overlaps forward与backward computation with the all-to-all comms they trigger. Bubbles drop, throughput climbs,与the keeping of two model-parameter copies (the "dual" that gives the name) is cheap once Expert Parallelism is already spreading experts across ranks anyway. This lesson is a Learn-type walkthrough of what DualPipe actually does与why Sea AI Lab's DualPipeV refinement drops the 2x parameter cost at the expense of a marginally tighter bubble.

**类型：** 学习
**语言：** Python (stdlib, schedule simulator)
**前置知识：** Phase 10 · 05 (distributed training, FSDP, DeepSpeed), Phase 10 · 14 (open-model architectures与MoE)
**时间：** 约 60 minutes

## 学习目标
- Name the four components of a DualPipe forward-backward chunk与why each one gets its own overlap window.
- Explain the pipeline bubble problem at scale,与what "bubble-free" means in practice versus in marketing.
- Trace a DualPipe schedule by hand for 8 PP ranks与16 micro-batches与confirm the forward与reverse streams fill each other's idle slots.
...
本课是 Phase 10「从零构建 LLM」的第 19 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
