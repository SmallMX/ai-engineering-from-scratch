...
> You built a GPT-2 Small from scratch in Lesson 04. Frontier open models in 2026 are the same family with five or six concrete changes. RMSNorm instead of LayerNorm. SwiGLU instead of GELU. RoPE instead of learned positions. GQA or MLA instead of full MHA. Mixture-of-Experts at scale. The math you already know covers 95% of them. This lesson reads Llama 3, DeepSeek-V3, Mixtral, Qwen,与Gemma side by side与names the exact line where each architecture diverges.

**类型：** 学习
**语言：** Python (stdlib)
**前置知识：** Phase 10, Lessons 04, 05, 12 (Pre-training, 扩展, 推理)
**时间：** 约 45 minutes

## 学习目标
- Read the config.json of Llama 3, Mistral, Mixtral, Gemma 2, Qwen 2.5,与DeepSeek-V3与explain every field
- Name the specific architectural change each model made versus GPT-2 Small与justify it from first principles
- Compute parameter count, KV cache size,与activation memory for any open model from its config alone
- Pick the right open model for a deployment target given latency, memory,与capability constraints

## 中文导读

本课是 Phase 10「从零构建 LLM」的第 14 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
