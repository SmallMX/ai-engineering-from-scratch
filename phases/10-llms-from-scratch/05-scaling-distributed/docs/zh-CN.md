# 扩展：Distributed 训练, FSDP, DeepSpeed
...
**类型：** 构建
**语言：** Python
**前置知识：** Phase 10, Lesson 04 (Pre-训练 a Mini GPT)
**时间：** 约 120 minutes

## 学习目标
- Explain the three types of parallelism (data, tensor, pipeline)与when each is necessary based on model与cluster size
...
- Configure FSDP or DeepSpeed ZeRO stages to shard model states across GPUs与fit models that exceed single-GPU memory

## 中文导读

本课是 Phase 10「从零构建 LLM」的第 05 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
