# Pre-训练 a Mini GPT (124M Parameters)

> GPT-2 Small has 124 million parameters. That's 12 transformer layers, 12 attention heads,与768-dimensional embeddings. You can train it from scratch on a single GPU in a few hours. Most people never do this. They use pre-trained checkpoints. But if you don't train one yourself, you don't actually understand what's happening inside the model you're building products on.

**类型：** 构建
**语言：** Python (with numpy)
**前置知识：** Phase 10, Lessons 01-03 (分词器, Building a 分词器, 数据 Pipelines)
**时间：** 约 120 minutes

## 学习目标
- Implement the full GPT-2 architecture (124M parameters) from scratch：token embeddings, positional embeddings, transformer blocks,与the language model head
- Train a GPT model on a text corpus using next-token prediction with cross-entropy loss
- Implement autoregressive text generation with temperature sampling与top-k/top-p filtering
- Monitor training loss curves与validate that the model learns coherent language patterns

## 中文导读

本课是 Phase 10「从零构建 LLM」的第 04 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
