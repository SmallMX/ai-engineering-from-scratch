...
> A base model predicts the next token. That's it. It doesn't follow instructions, answer questions, or refuse harmful requests. SFT is the bridge between a token predictor与a useful assistant. Every model you've ever talked to -- Claude, GPT, Llama Chat -- went through this step.
...
- Format training data using chat templates with system, user,与assistant roles,与mask loss on non-assistant tokens
...
本课是 Phase 10「从零构建 LLM」的第 06 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
