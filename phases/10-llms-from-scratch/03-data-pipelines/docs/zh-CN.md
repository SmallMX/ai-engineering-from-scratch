# 数据 Pipelines for Pre-训练

> The model is a mirror. It reflects whatever data you feed it. Feed it garbage, it reflects garbage with perfect fluency.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 10, Lessons 01-02 (分词器, Building a 分词器)
**时间：** 约 90 minutes

## 学习目标
- Build a streaming data pipeline that tokenizes, chunks, shuffles,与batches terabytes of text without loading it all into memory
- Implement data quality filters (deduplication, language detection, content filtering) used in real pre-training pipelines
- Create fixed-length training sequences with proper attention masks与document boundary handling
- Profile pipeline throughput to ensure the dataloader keeps up with GPU training speed

## 中文导读

本课是 Phase 10「从零构建 LLM」的第 03 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
