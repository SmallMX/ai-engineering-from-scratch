...
**类型：** 构建
**语言：** Python
**前置知识：** Phase 10, Lessons 01-08 (Transformer architecture, attention)
**时间：** 约 120 minutes

## 学习目标
- Implement KV-cache to eliminate redundant computation during autoregressive token generation
- Explain the prefill vs decode phases of LLM inference与why each has different bottlenecks (compute-bound vs memory-bound)
- Implement continuous batching与PagedAttention concepts to maximize GPU utilization under concurrent requests
- Compare inference optimization techniques (KV-cache, speculative decoding, flash attention)与their throughput/latency tradeoffs

## 中文导读

本课是 Phase 10「从零构建 LLM」的第 12 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
