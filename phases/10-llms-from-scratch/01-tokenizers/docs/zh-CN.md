...
**类型：** 构建
**语言：** Python
**前置知识：** Phase 05 (NLP 基础)
**时间：** 约 90 minutes

## 学习目标
- Implement BPE, WordPiece,与Unigram tokenization algorithms from scratch与compare their merge strategies
...
- Analyze tokenization artifacts across languages与code, identifying where specific tokenizers break down
- Use the tiktoken与sentencepiece libraries to tokenize text与inspect the resulting token IDs

## 中文导读

本课是 Phase 10「从零构建 LLM」的第 01 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
