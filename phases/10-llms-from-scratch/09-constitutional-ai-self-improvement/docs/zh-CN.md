# Constitutional AI与Self-Improvement

> RLHF needs humans in the loop. Constitutional AI replaces most of them with the model itself. Write a list of principles, have the model critique its own outputs against those principles,与train on the critiques. DeepSeek-R1 pushed this further in 2025：let the model generate millions of reasoning traces, grade them with a rule,与run GRPO on the outcome. Most of the "alignment work" in a 2026 frontier model is the model alignment itself. This lesson builds both loops.
...
- Derive the GRPO objective (DeepSeek-R1's group-relative policy optimization)与contrast it with PPO's value-function baseline
- Generate verifiable reasoning traces with rule-based outcome rewards与score them without a separate reward model
- Decide when self-improvement beats human preference data与when it collapses into mode seeking

## 中文导读

本课是 Phase 10「从零构建 LLM」的第 09 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
