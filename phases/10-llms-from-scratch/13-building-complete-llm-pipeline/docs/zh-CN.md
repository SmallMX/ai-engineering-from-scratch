...
- Define the artifact contract between stages：what each stage consumes, what it produces,与how the next stage verifies the input
- Build an orchestrator that tracks experiments, hashes artifacts,与gates ship decisions on eval thresholds
- Design the rollback plan：which artifacts are cheap to re-run, which are expensive,与what a corrupted checkpoint costs

## 中文导读

本课是 Phase 10「从零构建 LLM」的第 13 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
