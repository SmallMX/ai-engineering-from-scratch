# Speculative Decoding与EAGLE

> A frontier LLM generating one token requires a full forward pass over billions of parameters. That forward pass is massively over-provisioned：most of the time a much smaller model can guess the next 3-5 tokens correctly,与the big model only needs to *verify* the guess. When the guess is right you got 5 tokens for the price of one. Speculative decoding (Leviathan et al. 2023) made this exact,与EAGLE-3 (2025) pushed acceptance rates to ~4.5 tokens per verify：a 4-5x speedup at matched output distribution.

**类型：** 构建
**语言：** Python (with numpy)
**前置知识：** Phase 10 Lesson 12 (推理 Optimization), Phase 10 Lesson 04 (Pre-training Mini-GPT)
**时间：** 约 75 minutes

## 学习目标
- 理解 Speculative Decoding与EAGLE 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 10「从零构建 LLM」的第 25 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
