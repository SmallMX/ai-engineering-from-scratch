...
> Every autoregressive LLM from GPT-2 to Llama 3 trains on one loss per position：predict the next token. DeepSeek-V3 added a second loss per position：predict the token after that. The extra 14B of parameters (on a 671B model) got distilled back into the main model through gradient flow,与the trained MTP heads were repurposed at inference as speculative-decoding drafters with 80%+ acceptance. 1.8× generation throughput came for free. This lesson builds the sequential MTP module from the DeepSeek technical report, computes the loss与the shared-head parameter layout,与explains why MTP keeps the causal chain while Gloeckle et al.'s original parallel MTP broke it.

**类型：** 构建
**语言：** Python (stdlib)
**前置知识：** Phase 10 · 04 (pre-training a mini GPT), Phase 10 · 15 (speculative decoding)
**时间：** 约 60 minutes

## 学习目标
- State the MTP training objective与derive the joint loss across prediction depths.
- Explain the difference between Gloeckle et al.'s parallel MTP heads (2024)与DeepSeek-V3's sequential MTP modules与why the sequential design preserves the causal chain.
- Compute the parameter与memory overhead of adding MTP modules to a pre-training run.
- Implement one MTP module from scratch：the shared embedding, the per-depth transformer block, the projection,与the shared output head.

## 中文导读

本课是 Phase 10「从零构建 LLM」的第 18 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
