...
> State space models (SSMs)与transformers want different things. Transformer buy quality via attention at quadratic cost. SSMs buy linear-time inference与constant memory via a recurrence but lag quality. AI21's Jamba (March 2024)与Jamba 1.5 (August 2024) put them in the same model：1 Transformer layer for every 7 Mamba layers, MoE on every other block,与a 256k context window that fits on a single 80GB GPU. Mamba-3 (ICLR 2026) tightens the SSM side with complex-valued state spaces与MIMO projections. This lesson reads both architectures end to end与explains why the hybrid recipe has survived three years of scaling when pure-SSM与pure-Transformer long-context attempts have not.

**类型：** 学习
**语言：** Python (stdlib, layer-mix calculator)
**前置知识：** Phase 10 · 14 (open-model architectures), Phase 10 · 17 (native sparse attention)
**时间：** 约 60 minutes

## 学习目标
- Explain the three primitives in a Jamba block：Transformer layers, Mamba layers, MoE —与the 1:7:even interleaving recipe.
- State what an SSM's recurrence looks like at a high level与why it enables constant-memory inference.
- Compute the KV cache footprint of a Jamba model at 256k context与compare to what a pure-Transformer model would need.
- Name the three Mamba-3 innovations (exponential-trapezoidal discretization, complex-valued state update, MIMO)与the problem each one targets.

## 中文导读

本课是 Phase 10「从零构建 LLM」的第 21 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
