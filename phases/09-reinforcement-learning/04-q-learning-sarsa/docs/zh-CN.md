# Temporal Difference：Q-Learning与SARSA

> Monte Carlo waits until the episode ends. TD updates after every step by bootstrapping the next value estimate. Q-learning is off-policy与optimistic; SARSA is on-policy与cautious. Both are one line of code. Both underpin every deep-RL method in this phase.
...
## 学习目标
- 理解 Temporal Difference：Q-Learning与SARSA 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 9「强化学习」的第 04 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
