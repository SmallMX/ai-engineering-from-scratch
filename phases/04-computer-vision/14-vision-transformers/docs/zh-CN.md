...
> 把图像切成 patch，把 patch 当成 token，然后运行标准 transformer。
...
1. ViT 把图像切成 patches，并把 patch embeddings 当作 token。
2. positional embeddings 提供空间位置信息。
3. self-attention 建模 patch 之间的全局关系。
4. ViT 通常需要更多数据或强预训练。
5. patch size、image resolution 和 token count 直接影响计算量。
