...
> An image is a 2-D tensor. A video is a 3-D one. The theory is the same; the compute is 10-100x harder. OpenAI's Sora (Feb 2024) proved it was possible. By 2026 Veo 2, Kling 1.5, Runway Gen-3, Pika 2.0,与WAN 2.2 ship production video from text at 1080p —与the open-weights stack (CogVideoX, HunyuanVideo, Mochi-1, WAN 2.2) is 12 months behind.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 8 · 07 (Latent 扩散), Phase 7 · 09 (ViT), Phase 8 · 06 (DDPM)
**时间：** 约 45 minutes

## 学习目标
- 理解 视频 Generation 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 8「生成式 AI」的第 10 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
