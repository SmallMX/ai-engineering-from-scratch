...
> Most generators stir `z` into every layer at the same time. StyleGAN split it apart：first map `z` to an intermediate `w`, then *inject* `w` at every resolution level through AdaIN. That single change untangled the latent space与made photorealistic faces a solved problem for seven years running.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 8 · 03 (GAN), Phase 4 · 08 (Normalization), Phase 3 · 07 (CNNs)
**时间：** 约 45 minutes

## 学习目标
- 理解 StyleGAN 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 8「生成式 AI」的第 05 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
