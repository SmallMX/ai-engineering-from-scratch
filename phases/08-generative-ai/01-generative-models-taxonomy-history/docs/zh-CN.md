# Generative Models：Taxonomy与History

> Every image model, text model, video model,与3D model fits in one of five buckets. Pick the wrong bucket与you will fight the math for weeks. Pick the right one与the field's last twelve years of progress stacks cleanly in your head.

**类型：** 学习
**语言：** Python
**前置知识：** Phase 2 (ML 基础), Phase 3 (Deep Learning Core), Phase 7 · 14 (Transformer)
**时间：** 约 45 minutes

## 学习目标
- 理解 Generative Models：Taxonomy与History 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 8「生成式 AI」的第 01 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
