# MCP Sampling：Server-Requested LLM Completions与智能体 Loops

> Most MCP servers are dumb executors：take arguments, run code, return content. Sampling lets a server flip direction：it asks the client's LLM to make a decision. This enables server-hosted agent loops without the server owning any model credentials. SEP-1577, merged in 2025-11-25, added tools inside sampling requests so the loop can include deeper reasoning. Drift-risk note：the SEP-1577 tool-in-sampling shape was experimental through Q1 2026与is still settling in SDK API.

**类型：** 构建
**语言：** Python (stdlib, sampling harness)
**前置知识：** Phase 13 · 07 (MCP server), Phase 13 · 10 (resources与prompts)
**时间：** 约 75 minutes

## 学习目标
- Explain what `sampling/createMessage` solves (server-hosted loops without server-side API keys).
- Implement a server that asks the client to sample over a multi-turn prompt与returns the completion.
...
本课是 Phase 13「工具与协议」的第 11 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
