# The Tool Interface：Why 智能体 Need Structured I/O

> A language model produces tokens. A program takes actions. The gap between those two is the tool interface：a contract that lets the model request an action与the host execute it. Every 2026 stack：function calling on OpenAI, Anthropic,与Gemini; MCP's `tools/call`; A2A's task parts：is a different encoding of the same four-step loop. This lesson names the loop与shows the minimum machinery to run it.

**类型：** 学习
**语言：** Python (stdlib, no LLM)
**前置知识：** Phase 11 (LLM completion API)
**时间：** 约 45 minutes

## 学习目标
- Explain why an LLM that can only generate text cannot, on its own, take actions against the real world.
- Draw the four-step tool-call loop (describe → decide → execute → observe)与name who owns each step.
- Write a tool description as three parts：name, JSON Schema input,与a deterministic executor function.
- Distinguish pure与side-effecting tools与state why the split matters for safety.

## 中文导读

本课是 Phase 13「工具与协议」的第 01 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
