...
> "Ask the model nicely to return JSON" fails 5 to 15 percent of the time, even on frontier models. Structured outputs close that gap with constrained decoding：the model is literally prevented from emitting a token that would violate the schema. OpenAI's strict mode, Anthropic's schema-typed tool use, Gemini's `responseSchema`, Pydantic AI's `output_type`,与Zod's `.parse` are five surface forms of the same idea. This lesson builds the schema validator与the strict-mode contract learners will use for every production extraction pipeline.

**类型：** 构建
**语言：** Python (stdlib, JSON Schema 2020-12 subset)
**前置知识：** Phase 13 · 02 (function calling deep dive)
**时间：** 约 75 minutes

## 学习目标
- Write a JSON Schema 2020-12 for an extraction target using the right constraints (enum, min/max, required, pattern).
- Explain why strict mode与constrained decoding give different guarantees from "validate after generation".
- Distinguish the three failure modes：parse error, schema violation, model refusal.
- Ship an extraction pipeline with typed repair与typed refusal handling.

## 中文导读

本课是 Phase 13「工具与协议」的第 04 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
