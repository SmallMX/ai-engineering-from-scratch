# LLM 路由 Layer：LiteLLM, OpenRouter, Portkey

> Provider lock-in is expensive. Different tool-calling workloads suit different models. 路由 gateways give one API surface, retries, failover, cost tracking,与guardrails. Three archetypes dominate 2026：LiteLLM (open-source self-hosted), OpenRouter (managed SaaS), Portkey (production-grade, open-sourced in March 2026). This lesson names the decision criteria与walks a stdlib routing gateway.

**类型：** 学习
**语言：** Python (stdlib, routing + failover + cost tracker)
**前置知识：** Phase 13 · 02 (function calling), Phase 13 · 17 (gateways)
**时间：** 约 45 minutes

## 学习目标
- Distinguish self-hosted, managed,与production-grade routing options.
- Implement a fallback chain that retries on provider failures in a defined priority order.
- Track per-request cost与token usage across providers.
- Decide between LiteLLM, OpenRouter,与Portkey for a given production constraint.

## 中文导读

本课是 Phase 13「工具与协议」的第 21 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
