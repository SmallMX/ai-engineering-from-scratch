...
> Every integration before MCP was a one-off. The 模型上下文协议, first shipped by Anthropic in November 2024与now stewarded by the Linux Foundation's Agentic AI Foundation, standardizes discovery与invocation so any client can speak to any server. The 2025-11-25 spec names six primitives (three server, three client), a three-phase lifecycle,与a JSON-RPC 2.0 wire format. Learn those与the rest of the MCP chapter of this phase becomes reading.

**类型：** 学习
**语言：** Python (stdlib, JSON-RPC parser)
**前置知识：** Phase 13 · 01 through 05 (the tool interface与function calling)
**时间：** 约 45 minutes

## 学习目标
- Name all six MCP primitives (tools, resources, prompts on the server; roots, sampling, elicitation on the client)与give one use case each.
- Walk through the three-phase lifecycle (initialize, operation, shutdown)与state who sends which message at each phase.
- Parse与emit JSON-RPC 2.0 request, response,与notification envelopes.
- Explain what capability negotiation at `initialize` is与what breaks without it.

## 中文导读

本课是 Phase 13「工具与协议」的第 06 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
