...
> MCP is agent-to-tool. A2A (智能体2智能体) is agent-to-agent：an open protocol for letting opaque agents built on different frameworks collaborate. Released by Google in April 2025, donated to the Linux Foundation in June 2025, reaching v1.0 in April 2026 with 150+ supporters including AWS, Cisco, Microsoft, Salesforce, SAP,与ServiceNow. It absorbed IBM's ACP与added the AP2 payments extension. This lesson walks the 智能体 Card, Task lifecycle,与the two transport bindings.

**类型：** 构建
**语言：** Python (stdlib, Agent Card + Task harness)
**前置知识：** Phase 13 · 06 (MCP fundamentals), Phase 13 · 08 (MCP client)
**时间：** 约 75 minutes

## 学习目标
- Distinguish agent-to-tool (MCP) from agent-to-agent (A2A) use cases.
- Publish an 智能体 Card at `/.well-known/agent.json` with skills与endpoint metadata.
- Walk the Task lifecycle (submitted → working → input-required → completed / failed / canceled / rejected).
- Use Messages with Parts (text, file, data)与Artifacts as outputs.

## 中文导读

本课是 Phase 13「工具与协议」的第 19 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
