...
> An agent calls five tools, three MCP servers,与two sub-agents. You need one trace across all of it. The OpenTelemetry GenAI semantic conventions (stable attributes in v1.37与up) are the 2026 standard, natively supported by Datadog, Langfuse, Arize Phoenix, OpenLLMetry,与AgentOps. This lesson names the required attributes, walks the span hierarchy (agent → LLM → tool),与ships a stdlib span emitter you can plug into any OTel exporter.
...
## 学习目标
- Name the required OTel GenAI attributes for an LLM span与a tool-execution span.
- Build a trace hierarchy that covers agent loop, LLM call, tool call,与MCP client dispatch.
...
本课是 Phase 13「工具与协议」的第 20 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
