# Parallel Tool Calls与Streaming with 工具

> Three independent weather lookups serialized is three round trips. Run them in parallel与total time collapses to the slowest single call. Every frontier provider now emits multiple tool calls in a single turn. The payoff is real; the plumbing is subtle. This lesson walks both halves：the parallel fan-out与the streamed-argument reassembly, with emphasis on the id-correlation trap.
...
## 学习目标
- Explain why `parallel_tool_calls：true` exists与when to disable it.
...
本课是 Phase 13「工具与协议」的第 03 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
