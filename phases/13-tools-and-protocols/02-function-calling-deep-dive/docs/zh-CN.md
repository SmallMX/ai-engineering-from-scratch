# 函数调用 深入解析：OpenAI, Anthropic, Gemini

> The three frontier providers converged on the same tool-call loop in 2024与then diverged on everything else. OpenAI uses `tools`与`tool_calls`. Anthropic uses `tool_use`与`tool_result` blocks. Gemini uses `functionDeclarations`与unique-id correlation. This lesson diffs the three side by side so code that ships on one provider does not break when you port it.

**类型：** 构建
**语言：** Python (stdlib, schema translators)
**前置知识：** Phase 13 · 01 (the tool interface)
**时间：** 约 75 minutes

## 学习目标
- State the three shape differences between OpenAI, Anthropic,与Gemini function-calling payloads (declaration, call, result).
- Translate one tool declaration across all three provider formats与predict where strict-mode constraints will differ.
- Use `tool_choice` in each provider to force, forbid, or auto-pick tool calls.
- Know the per-provider hard limits (tool count, schema depth, argument length)与the error signatures each one emits when limits are violated.

## 中文导读

本课是 Phase 13「工具与协议」的第 02 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
