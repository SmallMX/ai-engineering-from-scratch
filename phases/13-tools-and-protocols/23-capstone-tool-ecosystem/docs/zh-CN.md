# 毕业项目：Build a Complete Tool Ecosystem

> Phase 13 taught every piece. This capstone wires them into one production-shaped system：an MCP server with tools + resources + prompts + tasks + UI, OAuth 2.1 at the edge, an RBAC gateway, a multi-server client, an A2A sub-agent call, OTel tracing into a collector, tool-poisoning detection in CI,与an AGENTS.md + SKILL.md bundle. By the end you can defend every architectural choice.

**类型：** 构建
**语言：** Python (stdlib, end-to-end ecosystem harness)
**前置知识：** Phase 13 · 01 through 21
**时间：** 约 120 minutes

## 学习目标
- Compose an MCP server exposing tools, resources, prompts,与a task with a `ui://` app.
- Front the server with an OAuth 2.1 gateway that enforces RBAC与pinned hashes.
...
本课是 Phase 13「工具与协议」的第 23 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
