...
> stdio works locally与nowhere else. Streamable HTTP (2025-03-26) is the remote standard. The old HTTP+SSE transport is deprecated与being removed in mid-2026. Picking the wrong transport costs a migration; picking the right one buys a remote-hostable MCP server with session continuity与DNS-rebinding protection.

**类型：** 学习
**语言：** Python (stdlib, Streamable HTTP endpoint skeleton)
**前置知识：** Phase 13 · 07, 08 (MCP server与client)
**时间：** 约 45 minutes

## 学习目标
- Pick between stdio与Streamable HTTP based on deployment shape (local vs remote, single-process vs fleet).
- Implement the Streamable HTTP single-endpoint pattern：POST for requests, GET for session stream.
- Enforce `Origin` validation与session-id semantics to defeat DNS-rebinding.
- Migrate a legacy HTTP+SSE server to Streamable HTTP before the mid-2026 removal deadlines.

## 中文导读

本课是 Phase 13「工具与协议」的第 09 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
