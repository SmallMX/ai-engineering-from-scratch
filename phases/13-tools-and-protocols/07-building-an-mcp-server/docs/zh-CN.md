...
> Most MCP tutorials show only stdio hello-worlds. A real server exposes tools plus resources plus prompts, handles capability negotiation, emits structured errors,与works the same across SDKs. This lesson builds a notes server end-to-end：stdlib stdio transport, JSON-RPC dispatch, the three server primitives,与a pure-function style that drops into either the Python SDK's FastMCP or the TypeScript SDK when you graduate.

**类型：** 构建
**语言：** Python (stdlib, stdio MCP server)
**前置知识：** Phase 13 · 06 (MCP fundamentals)
**时间：** 约 75 minutes

## 学习目标
- Implement `initialize`, `tools/list`, `tools/call`, `resources/list`, `resources/read`, `prompts/list`,与`prompts/get` methods.
- Write a dispatch loop that reads JSON-RPC messages from stdin与writes responses to stdout.
- Emit structured error responses per the JSON-RPC 2.0 spec与MCP's additional codes.
- Graduate a stdlib implementation to FastMCP (Python SDK) or the TypeScript SDK without rewriting tool logic.

## 中文导读

本课是 Phase 13「工具与协议」的第 07 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
