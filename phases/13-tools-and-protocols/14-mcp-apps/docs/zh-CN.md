...
> Text-only tool output caps what agents can show. MCP Apps (SEP-1724, official January 26, 2026) let a tool return sandboxed interactive HTML rendered inline in Claude Desktop, ChatGPT, Cursor, Goose,与VS Code. Dashboards, forms, maps, 3D scenes, all through one extension. This lesson walks the `ui://` resource scheme, the `text/html;profile=mcp-app` MIME, the iframe-sandbox postMessage protocol,与the security surface that comes with letting a server render HTML.

**类型：** 构建
**语言：** Python (stdlib, UI resource emitter), HTML (sample app)
**前置知识：** Phase 13 · 07 (MCP server), Phase 13 · 10 (resources)
**时间：** 约 75 minutes

## 学习目标
- Return a `ui://` resource from a tool call与set the correct MIME与metadata.
- Declare a tool's associated UI with `_meta.ui.resourceUri`, `_meta.ui.csp`,与`_meta.ui.permissions`.
- Implement the iframe sandbox postMessage JSON-RPC for UI-to-host communication.
- Apply CSP与permissions-policy defaults that defend against UI-originated attacks.

## 中文导读

本课是 Phase 13「工具与协议」的第 14 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
