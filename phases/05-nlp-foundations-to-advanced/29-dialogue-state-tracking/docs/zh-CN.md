...
> "I want a cheap restaurant in the north... actually make it moderate...与add Italian." Three turns, three state updates. DST keeps the slot-value dict in sync so the booking works.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 5 · 17 (Chatbots), Phase 5 · 20 (结构化输出)
**时间：** 约 75 minutes

## 学习目标
- 理解 Dialogue State Tracking 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 5「NLP：从基础到进阶」的第 29 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
