# Chatbots：Rule-Based to Neural to LLM 智能体

> ELIZA replied with pattern matches. DialogFlow mapped intents. GPT answered from weights. Claude runs tools与verifies. Each era solved the previous one's worst failure.

**类型：** 学习
**语言：** Python
**前置知识：** Phase 5 · 13 (问答), Phase 5 · 14 (Information 检索)
**时间：** 约 75 minutes

## 学习目标
- 理解 Chatbots：Rule-Based to Neural to LLM 智能体 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 5「NLP：从基础到进阶」的第 17 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
