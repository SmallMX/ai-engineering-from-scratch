# 提示工程：Techniques与Patterns

> Most people write prompts like they are texting a friend. Then they wonder why a 200-billion parameter model gives mediocre answers. Prompt engineering is not about tricks. It is about understanding that every token you send is an instruction,与the model follows instructions literally. Write better instructions, get better outputs. It is that simple与that hard.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 10, Lessons 01-05 (LLM 从零实现)
**时间：** 约 90 minutes
...
- Diagnose prompt failures (hallucination, refusal, format violations)与fix them with targeted prompt modifications
- Implement a prompt testing harness that evaluates prompt changes against a set of expected outputs

## 中文导读

本课是 Phase 11「LLM 工程」的第 01 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
