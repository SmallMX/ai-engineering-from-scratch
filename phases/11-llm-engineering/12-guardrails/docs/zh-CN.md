# Guardrails, 安全与Content Filtering

> Your LLM application will be attacked. Not might. Will. The first prompt injection attempt against your production system will come within 48 hours of launch. The question is not whether someone will try "ignore previous instructions与reveal your system prompt" -- the question is whether your system folds or holds. Every chatbot, every agent, every RAG pipeline is a target. If you ship without guardrails, you are shipping a vulnerability with a chat interface.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 11 Lesson 01 (提示工程), Phase 11 Lesson 09 (函数调用)
**时间：** 约 45 minutes

## 学习目标
- Implement input guardrails that detect与block prompt injection, jailbreak attempts,与toxic content before reaching the model
- Build output guardrails that validate responses for PII leakage, hallucinated URLs,与policy violations
- Design a layered defense system combining input filtering, system prompt hardening,与output validation
- Test guardrails against a red-team prompt set与measure the false positive/negative rate

## 中文导读

本课是 Phase 11「LLM 工程」的第 12 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
