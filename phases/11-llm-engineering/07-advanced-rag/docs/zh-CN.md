# 进阶 RAG (Chunking, Reranking, Hybrid 搜索)

> Basic RAG retrieves the top-k most similar chunks. That works for simple questions. It falls apart for multi-hop reasoning, ambiguous queries,与large corpora. 进阶 RAG is the difference between a demo that works on 10 documents与a system that works on 10 million.
...
## 学习目标
- Implement advanced chunking strategies (semantic, recursive, parent-child) that preserve document structure与context
- Build a hybrid search pipeline combining BM25 keyword matching with semantic vector search与a cross-encoder reranker
- Apply query transformation techniques (HyDE, multi-query, step-back) to improve retrieval on ambiguous or complex questions
- Diagnose与fix common RAG failures：wrong chunk retrieved, answer not in context, multi-hop reasoning breakdown

## 中文导读

本课是 Phase 11「LLM 工程」的第 07 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
