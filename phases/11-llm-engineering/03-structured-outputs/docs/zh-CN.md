...
> Your LLM returns a string. Your application needs JSON. That gap has crashed more production systems than any model hallucination. Structured output is the bridge between natural language与typed data. Get it right与your LLM becomes a reliable API. Get it wrong与you're parsing free-text with regex at 3am.
...
## 学习目标
- Implement JSON-mode与schema-constrained outputs using OpenAI与Anthropic API parameters
- Build a Pydantic validation layer that rejects malformed LLM outputs与retries with error feedback
...
本课是 Phase 11「LLM 工程」的第 03 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
