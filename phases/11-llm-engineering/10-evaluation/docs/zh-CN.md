# 评估与Testing LLM Applications

> You would never deploy a web app without tests. You would never ship a database migration without a rollback plan. But right now, most teams ship LLM applications by reading 10 outputs与saying "yeah, looks good." That is not evaluation. That is hope. Hope is not an engineering practice. Every prompt change, every model swap, every temperature tweak changes your output distribution in ways you cannot predict by reading a handful of examples. 评估 is the only thing standing between your application与silent degradation.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 11 Lesson 01 (提示工程), Lesson 09 (函数调用)
**时间：** 约 45 minutes

## 学习目标
- Build an evaluation dataset with input-output pairs, rubrics,与edge cases specific to your LLM application
- Implement automated scoring using LLM-as-judge, regex matching,与deterministic assertion checks
...
本课是 Phase 11「LLM 工程」的第 10 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
