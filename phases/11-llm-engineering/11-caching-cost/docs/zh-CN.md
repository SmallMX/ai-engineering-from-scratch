# 缓存, Rate Limiting与Cost Optimization
...
- Calculate per-request costs across providers与implement token-aware rate limiting与budget alerts
- Build a cost optimization layer with prompt compression, model routing (expensive vs cheap),与response caching
- Design a tiered caching strategy using exact match, semantic similarity,与prefix caching for different query types

## 中文导读

本课是 Phase 11「LLM 工程」的第 11 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
