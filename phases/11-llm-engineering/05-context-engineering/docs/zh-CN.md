# 上下文工程：Windows, Budgets, 记忆,与检索

> Prompt engineering is a subset. Context engineering is the whole game. A prompt is a string you type. Context is everything that goes into the model's window：system instructions, retrieved documents, tool definitions, conversation history, few-shot examples,与the prompt itself. The best AI engineers in 2026 are context engineers. They decide what goes in, what stays out,与in what order.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 10 (LLM 从零实现), Phase 11 Lesson 01-02
**时间：** 约 90 minutes
...
- Implement context window management strategies：truncation, summarization,与sliding window for conversation history
- Prioritize与order context components to maximize the model's attention on the most relevant information
- Build a context assembler that dynamically allocates tokens based on query type与available window space

## 中文导读

本课是 Phase 11「LLM 工程」的第 05 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
