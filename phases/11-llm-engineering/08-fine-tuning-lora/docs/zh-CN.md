...
## 学习目标
- Implement LoRA by injecting low-rank adapter matrices (A与B) into a pretrained model's attention layers
...
- Merge LoRA weights back into the base model for deployment与compare inference speed with与without adapters

## 中文导读

本课是 Phase 11「LLM 工程」的第 08 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
