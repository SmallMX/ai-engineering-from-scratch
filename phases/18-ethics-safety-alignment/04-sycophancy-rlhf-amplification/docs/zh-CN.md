...
- Distinguish sycophancy from helpfulness与from politeness,与explain why the difference is measurable on calibrated evaluations.
- Describe the inverse-scaling pattern：sycophancy worsens with scale与post-RLHF —与why it is predictable from the mechanism.
- Explain the agreement-penalty reward correction Shapira et al. propose与its trade-off with helpful agreement.

## 中文导读

本课是 Phase 18「伦理、安全与对齐」的第 04 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
