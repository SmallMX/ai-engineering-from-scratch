# EchoLeak与the Emergence of CVEs for AI

> CVE-2025-32711 "EchoLeak" (CVSS 9.3) was the first publicly documented zero-click prompt injection in a production LLM system (Microsoft 365 Copilot). Discovered by Aim Labs (Aim 安全), disclosed to MSRC, patched via server-side update June 2025. Attack：attacker sends a crafted email to any employee; the victim's Copilot retrieves the email as RAG context during a routine query; hidden instructions execute; Copilot exfiltrates sensitive organizational data via a CSP-approved Microsoft domain. Bypassed XPIA prompt-injection filters与Copilot's link-redaction mechanisms. Aim Labs's term："LLM Scope Violation"：external untrusted input manipulates the model to access与leak confidential data. Related：CamoLeak (CVSS 9.6, GitHub Copilot Chat) exploited the Camo image proxy; fixed by disabling image rendering entirely. GitHub Copilot RCE CVE-2025-53773. NIST has called indirect prompt injection "generative AI's greatest security flaw"; OWASP 2025 ranks it #1 threat to LLM applications.

**类型：** 学习
**语言：** Python (stdlib, scope-violation trace reconstruction)
**前置知识：** Phase 18 · 15 (indirect prompt injection)
**时间：** 约 45 minutes

## 学习目标
- Describe the EchoLeak attack chain from email delivery to data exfiltration.
- Define "LLM Scope Violation"与explain why it is a new vulnerability class.
- Describe the three related CVEs (EchoLeak, CamoLeak, Copilot RCE)与what each reveals about the production attack surface.
...
本课是 Phase 18「伦理、安全与对齐」的第 25 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
