...
## 学习目标
- Describe the OpenAI 内容审核 API's category taxonomy与how it differs from Llama Guard 3's MLCommons set.
- Describe the three moderation-layer pattern (input, output, custom)与name one failure mode of each.
- Describe Perspective API's position as a pre-LLM-era baseline与why it remains used in research.
- State the Azure deprecation timeline.

## 中文导读

本课是 Phase 18「伦理、安全与对齐」的第 29 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
