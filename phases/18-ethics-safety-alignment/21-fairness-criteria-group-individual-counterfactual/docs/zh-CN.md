...
**前置知识：** Phase 18 · 20 (bias), Phase 02 (classical ML)
**时间：** 约 60 minutes

## 学习目标
- State the three group-fairness criteria (demographic parity, equalized odds, conditional use accuracy equality)与one impossibility result.
- Describe individual fairness via the Dwork et al. 2012 Lipschitz formulation.
- Describe counterfactual fairness与its causal-graph dependency.
- Explain backtracking counterfactuals与why they sidestep the intervention-on-protected-attribute problem.

## 中文导读

本课是 Phase 18「伦理、安全与对齐」的第 21 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
