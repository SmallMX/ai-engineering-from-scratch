# Why Transformer：The Problems with RNNs

> RNNs process tokens one at a time. Transformer process all tokens at once. That single architectural bet changed every scaling curve in deep learning after 2017.

**类型：** 学习
**语言：** Python
**前置知识：** Phase 3 (Deep Learning Core), Phase 5 · 09 (Sequence-to-Sequence), Phase 5 · 10 (注意力 Mechanism)
**时间：** 约 45 minutes

## 学习目标
- 理解 Why Transformer：The Problems with RNNs 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 7「Transformer 深入解析」的第 01 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
