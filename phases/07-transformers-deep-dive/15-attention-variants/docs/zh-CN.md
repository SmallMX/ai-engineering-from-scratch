# 注意力 Variants：Sliding Window, Sparse, Differential

> Full attention is a circle. Every token sees every token,与memory pays the price. Four variants bend the shape of the circle与recover half the cost.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 7 · 02 (Self-注意力), Phase 7 · 03 (Multi-Head), Phase 7 · 12 (KV 缓存 / Flash 注意力)
**时间：** 约 60 minutes

## 学习目标
- 理解 注意力 Variants：Sliding Window, Sparse, Differential 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 7「Transformer 深入解析」的第 15 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
