...
> The 2020 Kaplan paper said：bigger model, lower loss. The 2022 Hoffmann paper said：you were under-training. Compute goes into two buckets：parameters与tokens —与the split is not obvious.
...
## 学习目标
- 理解 扩展 Laws 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 7「Transformer 深入解析」的第 13 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
