...
> 注意力 is a lookup table where every word asks "who matters to me?" -与learns the answer.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 3 (Deep Learning Core), Phase 5 Lesson 10 (Sequence-to-Sequence)
**时间：** 约 90 minutes

## 学习目标
- Implement scaled dot-product self-attention from scratch using only NumPy, including query/key/value projections与the softmax-weighted sum
- Build a multi-head attention layer that splits heads, computes parallel attention,与concatenates results
- Trace how the attention matrix captures token relationships与explain why scaling by sqrt(d_k) prevents softmax saturation
- Apply causal masking to convert bidirectional attention into autoregressive (decoder-style) attention

## 中文导读

本课是 Phase 7「Transformer 深入解析」的第 02 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
