# 记忆 Blocks与Sleep-Time Compute (Letta)

> MemGPT became Letta in 2024. The 2026 evolution adds two ideas：discrete functional memory blocks the model can edit directly,与a sleep-time agent that consolidates memory asynchronously while the primary agent is idle. This is how you scale memory beyond one conversation.
...
## 学习目标
- Name the three memory tiers Letta uses (core, recall, archival)与the role of each.
- Explain the memory-block pattern：Human block, Persona block,与user-defined blocks as first-class typed objects.
- Describe what sleep-time compute is, why it sits off the critical path,与why it can run a stronger model than the primary agent.
- Implement a scripted two-agent loop where a primary agent serves responses与a sleep-time agent consolidates blocks between turns.

## 中文导读

本课是 Phase 14「智能体工程」的第 08 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
