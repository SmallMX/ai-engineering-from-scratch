...
> Three production computer-use models in 2026. All three are vision-based. All three treat screenshots, DOM text,与tool outputs as untrusted input. Only direct user instructions count as permission. Per-step safety services are the norm.
...
- Explain the per-step safety pattern Gemini 2.5 计算机使用 documents.
- Summarize the untrusted-input contract all three models enforce.

## 中文导读

本课是 Phase 14「智能体工程」的第 21 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
