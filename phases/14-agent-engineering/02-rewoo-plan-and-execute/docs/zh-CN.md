# ReWOO与Plan-and-Execute：Decoupled 规划

> ReAct interleaves thought与action in one stream. ReWOO separates them：one big plan up front, then execute. 5x fewer tokens, +4% accuracy on HotpotQA,与you can distill the planner into a 7B model. Plan-and-Execute generalized it; Plan-and-Act scaled it to web navigation.

**类型：** 构建
**语言：** Python (stdlib)
**前置知识：** Phase 14 · 01 (智能体 Loop)
**时间：** 约 60 minutes

## 学习目标
- Explain why ReWOO's Planner / Worker / Solver split saves tokens与improves robustness over ReAct's interleaved loop.
- Implement a plan DAG, a dependency-ordered executor,与a solver that composes worker outputs：all stdlib.
...
本课是 Phase 14「智能体工程」的第 02 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
