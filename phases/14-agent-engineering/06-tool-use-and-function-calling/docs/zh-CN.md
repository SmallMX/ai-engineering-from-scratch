...
> Toolformer (Schick et al., 2023) started self-supervised tool annotation. Berkeley 函数调用 Leaderboard V4 (Patil et al., 2025) sets the 2026 bar：40% agentic, 30% multi-turn, 10% live, 10% non-live, 10% hallucination. Single-turn is solved. 记忆, dynamic decision-making,与long-horizon tool chains are not.

**类型：** 构建
**语言：** Python (stdlib)
**前置知识：** Phase 14 · 01 (智能体 Loop), Phase 13 · 01 (函数调用 深入解析)
**时间：** 约 60 minutes

## 学习目标
- Explain Toolformer's self-supervised training signal：keep tool annotations only when execution reduces next-token loss.
- Name BFCL V4's five evaluation categories与what each measures.
- Implement a stdlib tool registry with schema validation, argument coercion,与execution sandboxing.
- Diagnose the three 2026 open problems：long-horizon tool chaining, dynamic decision-making,与memory.

## 中文导读

本课是 Phase 14「智能体工程」的第 06 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
