# 规划 with HTN与Evolutionary 搜索

> Symbolic planning handles the cases where the plan is provably correct. Evolutionary code search handles the cases where the fitness function is machine-checkable. ChatHTN (2025)与AlphaEvolve (2025) show what each unlocks when paired with an LLM.

**类型：** 构建
**语言：** Python (stdlib)
**前置知识：** Phase 14 · 02 (ReWOO与Plan-and-Execute)
**时间：** 约 75 minutes
...
- Explain AlphaEvolve's evolutionary loop与why it only works with a programmatic evaluator.
- Implement a toy HTN planner plus a toy evolutionary search in stdlib.

## 中文导读

本课是 Phase 14「智能体工程」的第 11 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
