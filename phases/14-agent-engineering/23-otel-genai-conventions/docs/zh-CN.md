...
> OpenTelemetry's GenAI SIG (launched April 2024) defines the standard schema for agent telemetry. Span names, attributes,与content-capture rules converge across vendors so agent traces mean the same thing in Datadog, Grafana, Jaeger,与Honeycomb.

**类型：** Learn + Build
**语言：** Python (stdlib)
**前置知识：** Phase 14 · 13 (LangGraph), Phase 14 · 24 (可观测性 Platforms)
**时间：** 约 60 minutes

## 学习目标
- Name the GenAI span categories：model/client, agent, tool.
- Distinguish `invoke_agent` CLIENT vs INTERNAL spans与when each applies.
...
本课是 Phase 14「智能体工程」的第 23 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
