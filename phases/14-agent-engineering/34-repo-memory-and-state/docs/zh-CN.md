# Repo 记忆与Durable State

> Chat history is volatile. The repo is durable. The workbench stores agent state in versioned files so the next session, the next agent,与the next reviewer all read from the same source of truth.

**类型：** 构建
**语言：** Python (stdlib + `jsonschema` optional)
**前置知识：** Phase 14 · 32 (Minimal Workbench)
**时间：** 约 60 minutes

## 学习目标
- Define what belongs in repo memory与what belongs in chat history.
- Author JSON Schemas for `agent_state.json`与`task_board.json`.
- Build a state manager that loads, validates, mutates,与persists state atomically.
- Use the schema to refuse bad writes before they corrupt the workbench.

## 中文导读

本课是 Phase 14「智能体工程」的第 34 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
