...
**类型：** Learn + Build
**语言：** Python (stdlib)
**前置知识：** Phase 14 · 12 (Workflow Patterns), Phase 14 · 05 (Self-Refine与CRITIC)
**时间：** 约 60 minutes

## 学习目标
- Explain the debate protocol：N proposers, R rounds, converge on a shared answer.
- Describe why debate improves factuality, rule-following,与reasoning.
- Explain sparse topology：not every debater needs to see every other.
- Implement a stdlib debate over a scripted LLM with full-mesh与sparse variants; measure token cost vs accuracy.

## 中文导读

本课是 Phase 14「智能体工程」的第 25 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
