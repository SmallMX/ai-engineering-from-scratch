# Tree of Thoughts与LATS：Deliberate 搜索

> A single chain-of-thought trajectory has no room to backtrack. ToT (Yao et al., 2023) turns reasoning into a tree with self-evaluation on each node. LATS (Zhou et al., 2024) unifies ToT with ReAct与Reflexion under Monte Carlo Tree 搜索. Game of 24 goes from 4% (CoT) to 74% (ToT); LATS hits 92.7% pass@1 on HumanEval.
...
- Decide when search is worth the token multiplier (Game of 24, code generation)与when a single trajectory is enough (simple Q&A).

## 中文导读

本课是 Phase 14「智能体工程」的第 04 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
