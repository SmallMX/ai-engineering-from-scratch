# 毕业项目：Ship a Reusable 智能体 Workbench Pack

> The mini-track ends with a pack you drop into any repo. Eleven lessons of surfaces compressed into a directory you can `cp -r`与have an agent working reliably the next morning. The capstone is the artifact this curriculum trades on.

**类型：** 构建
**语言：** Python (stdlib)
**前置知识：** Phases 14 · 31 to 14 · 41
**时间：** 约 75 minutes

## 学习目标
- Package the seven workbench surfaces into one drop-in directory.
- Pin the schemas, scripts,与templates so a new repo gets a known-good baseline.
- Add a single installer script that lays down the pack idempotently.
- Decide what stays in the pack与what stays out, defending the cut for each.

## 中文导读

本课是 Phase 14「智能体工程」的第 42 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
