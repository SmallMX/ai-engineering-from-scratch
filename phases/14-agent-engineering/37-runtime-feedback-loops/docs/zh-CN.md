...
> 智能体 that do not see real command output guess. A feedback runner captures stdout, stderr, exit code,与timing into a structured record the next turn can read. Then the agent reacts to facts instead of to its own prediction of facts.

**类型：** 构建
**语言：** Python (stdlib)
**前置知识：** Phase 14 · 32 (Minimal Workbench), Phase 14 · 35 (Init Script)
**时间：** 约 50 minutes

## 学习目标
- Distinguish runtime feedback from observability telemetry.
- Build a feedback runner that wraps shell commands与persists structured records.
...
本课是 Phase 14「智能体工程」的第 37 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
