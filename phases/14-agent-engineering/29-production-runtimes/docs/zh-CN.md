# 生产 Runtimes：Queue, Event, Cron

> 生产 agents run on six runtime shapes：request-response, streaming, durable execution, queue-based background, event-driven,与scheduled. Pick the shape before you pick the framework. 可观测性 is load-bearing at every shape.
...
## 学习目标
- Name the six production runtime shapes与match each to a framework / product pattern.
- Explain why durable execution (LangGraph) matters for long-horizon tasks.
- Describe the event-driven runtime与when Claude Managed 智能体 fits.
- Explain the observability-as-load-bearing claim for multi-step agents.

## 中文导读

本课是 Phase 14「智能体工程」的第 29 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
