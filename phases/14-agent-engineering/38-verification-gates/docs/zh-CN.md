...
> The agent does not get to mark its own work as done. A verification gate reads the scope contract, the feedback log, the rule report,与the diff,与answers a single question：is this task actually complete? If the gate says no, the task is not done, no matter what the chat says.

**类型：** 构建
**语言：** Python (stdlib)
**前置知识：** Phase 14 · 33 (Rules), Phase 14 · 36 (Scope), Phase 14 · 37 (Feedback)
**时间：** 约 55 minutes

## 学习目标
- Define a verification gate as a deterministic function over workbench artifacts.
- Combine rule report, scope report, feedback records,与diff into a single verdict.
- Emit a `verification_report.json` the reviewer agent与CI can both read.
- Refuse to advance a task on any block-severity failure, without exception.

## 中文导读

本课是 Phase 14「智能体工程」的第 38 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
