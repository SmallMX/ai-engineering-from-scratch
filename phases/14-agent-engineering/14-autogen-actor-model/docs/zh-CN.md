# AutoGen v0.4：Actor Model与智能体 框架

> AutoGen v0.4 (Microsoft Research, Jan 2025) redesigned agent orchestration around the actor model. Async message exchange, event-driven agents, fault isolation, natural concurrency. The framework is now in maintenance mode while Microsoft 智能体 框架 (public preview Oct 2025) becomes the successor.
...
- Name AutoGen v0.4's three API layers：Core, AgentChat, Extensions —与what each is for.
- Explain why decoupling message delivery from handling gives fault isolation与natural concurrency.
- Implement a stdlib actor runtime in Python与port a two-agent code-review flow onto it.

## 中文导读

本课是 Phase 14「智能体工程」的第 14 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
