# OpenAI 智能体 SDK：Handoffs, Guardrails, Tracing

> OpenAI 智能体 SDK is the lightweight multi-agent framework built on the Responses API. Five primitives：智能体, Handoff, Guardrail, Session, Tracing. Handoffs are tools named `transfer_to_<agent>`. Guardrails trip on input or output. Tracing is on by default.
...
## 学习目标
- Name the five primitives of the OpenAI 智能体 SDK.
- Explain handoffs：why they are modeled as tools, what name shape the model sees,与how context transfers.
- Distinguish input guardrails, output guardrails,与tool guardrails; explain `run_in_parallel` vs blocking mode.
- Implement a stdlib runtime with handoffs + guardrails + span-style tracing.

## 中文导读

本课是 Phase 14「智能体工程」的第 16 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
