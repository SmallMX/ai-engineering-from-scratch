# Claude 智能体 SDK：Subagents与Session Store

> The Claude 智能体 SDK is the library form of the Claude Code harness. Built-in tools, subagents for context isolation, hooks, W3C trace propagation, session store parity. Claude Managed 智能体 is the hosted alternative for long-running async work.

**类型：** Learn + Build
**语言：** Python (stdlib)
**前置知识：** Phase 14 · 01 (智能体 Loop), Phase 14 · 10 (Skill Libraries)
**时间：** 约 75 minutes

## 学习目标
- Explain the difference between the Anthropic Client SDK (raw API)与the Claude 智能体 SDK (harness shape).
- Describe subagents：parallelization与context isolation —与when to reach for them.
- Name the Python SDK's session store surface (`append`, `load`, `list_sessions`, `delete`, `list_subkeys`)与the role of `--session-mirror`.
- Implement a stdlib harness with built-in tools, subagent spawning with isolated context, lifecycle hooks,与a session store.

## 中文导读

本课是 Phase 14「智能体工程」的第 17 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
