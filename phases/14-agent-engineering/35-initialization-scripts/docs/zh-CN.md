# Initialization Scripts for 智能体

> Every session that starts cold pays a tax. The agent reads the same files, retries the same probes,与rediscovers the same paths. An init script pays the tax once与writes the answers into state.

**类型：** 构建
**语言：** Python (stdlib)
**前置知识：** Phase 14 · 32 (Minimal Workbench), Phase 14 · 34 (Repo 记忆)
**时间：** 约 45 minutes

## 学习目标
- Identify the work an agent should never have to redo per session.
- Build a deterministic init script that probes runtime, dependencies,与repo health.
- Persist the probe result so the agent reads it instead of re-running checks.
- Fail loud, fast,与with one place to look when initialization fails.

## 中文导读

本课是 Phase 14「智能体工程」的第 35 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
