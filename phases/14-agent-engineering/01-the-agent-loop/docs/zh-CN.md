# The 智能体 Loop：Observe, Think, Act

> Every agent in 2026：Claude Code, Cursor, Devin, Operator：is a variant of the ReAct loop from 2022. 推理 tokens interleave with tool calls与observations until a stop condition fires. Learn this loop cold before touching any framework.
...
## 学习目标
- Name the three parts of the ReAct loop：Thought, Action, Observation —与explain why each one is load-bearing.
- Implement a stdlib agent loop with a toy LLM, tool registry,与stop condition under 200 lines.
...
- Explain why every modern harness (Claude 智能体 SDK, OpenAI 智能体 SDK, LangGraph, AutoGen v0.4) still runs this loop under the hood.

## 中文导读

本课是 Phase 14「智能体工程」的第 01 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
