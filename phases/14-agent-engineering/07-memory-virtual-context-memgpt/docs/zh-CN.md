# 记忆：Virtual Context与MemGPT

> Context windows are finite. Conversations, documents,与tool traces are not. MemGPT (Packer et al., 2023) frames this as OS virtual memory：main context is RAM, external store is disk, the agent pages between them. This is the pattern every 2026 memory system inherits.

**类型：** 构建
**语言：** Python (stdlib)
**前置知识：** Phase 14 · 01 (智能体 Loop), Phase 14 · 06 (Tool Use)
**时间：** 约 75 minutes

## 学习目标
- Explain the OS analogy MemGPT builds on：main context = RAM, external context = disk, memory tools = page in/out.
- Implement the two-tier MemGPT pattern in stdlib with a main-context buffer, an external searchable store,与page in/out tools.
- Describe how the agent issues "interrupts" to query or modify external memory与how the result is spliced back into the next prompt.
- Identify the MemGPT design choices that carry into Letta (Lesson 08)与Mem0 (Lesson 09).

## 中文导读

本课是 Phase 14「智能体工程」的第 07 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
