# The Minimal 智能体 Workbench

> The smallest useful workbench is three files：a root instructions router, a state file,与a task board. Everything else is layered on top. If a repo cannot carry these three, no model will save it.

**类型：** 构建
**语言：** Python (stdlib)
**前置知识：** Phase 14 · 31 (Why Capable Models Still Fail)
**时间：** 约 45 minutes
...
- Build a state file the agent can read at every turn与write at the end.
- Build a task board that survives multi-session work without chat history.

## 中文导读

本课是 Phase 14「智能体工程」的第 32 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
