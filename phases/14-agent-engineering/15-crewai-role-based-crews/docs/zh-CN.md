# CrewAI：Role-Based Crews与Flows

> CrewAI is the 2026 role-based multi-agent framework. Four primitives：智能体, Task, Crew, Process. Two top-level shapes：Crews (autonomous, role-based collaboration)与Flows (event-driven, deterministic). The docs are blunt："for any production-ready application, start with a Flow."

**类型：** Learn + Build
**语言：** Python (stdlib)
**前置知识：** Phase 14 · 12 (Workflow Patterns), Phase 14 · 14 (Actor Model)
**时间：** 约 75 minutes

## 学习目标
- Name CrewAI's four primitives (智能体, Task, Crew, Process)与what each owns.
- Distinguish Sequential, Hierarchical,与the planned Consensus process; pick one per workload.
- Distinguish Crews (autonomous role-based) from Flows (event-driven deterministic),与explain the docs' production recommendation.
- Wire tools with the `@tool` decorator与`BaseTool` subclass; reason about structured outputs vs free text.
- Name the four CrewAI memory types与when each pays off.
- Implement a stdlib three-agent crew (researcher, writer, editor) that produces a brief.

## 中文导读

本课是 Phase 14「智能体工程」的第 15 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
