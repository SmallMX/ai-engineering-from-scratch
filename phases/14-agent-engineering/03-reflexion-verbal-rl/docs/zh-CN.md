...
> Gradient-based RL needs thousands of trials与a GPU cluster to fix a failure mode. Reflexion (Shinn et al., NeurIPS 2023) does it in natural language：after each failed trial, the agent writes a reflection, stores it in episodic memory,与conditions the next trial on that memory. This is the pattern behind Letta's sleep-time compute, Claude Code's CLAUDE.md learnings,与pro-workflow's learn-rule.

**类型：** 构建
**语言：** Python (stdlib)
**前置知识：** Phase 14 · 01 (智能体 Loop), Phase 14 · 02 (ReWOO)
**时间：** 约 60 minutes

## 学习目标
- Name the three components of Reflexion (Actor, Evaluator, Self-Reflector)与the role of episodic memory.
- Implement a stdlib Reflexion loop with binary evaluator, reflection buffer,与fresh re-attempts.
- Choose between scalar, heuristic,与self-evaluated feedback sources for a given task.
- Explain why verbal reinforcement catches errors that gradient-based RL would need thousands of trials to fix.

## 中文导读

本课是 Phase 14「智能体工程」的第 03 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
