# Reviewer 智能体：Separate Builder from Marker

> The agent that wrote the code cannot grade it. A reviewer is a second loop with a different system prompt, a different goal,与read-only access to everything the builder produced. The gap between builder与reviewer is where most reliability lives.

**类型：** 构建
**语言：** Python (stdlib)
**前置知识：** Phase 14 · 38 (Verification Gate)
**时间：** 约 55 minutes

## 学习目标
- State why the same agent cannot reliably review its own work.
- Build a reviewer agent loop that consumes builder artifacts与emits a structured review report.
...
本课是 Phase 14「智能体工程」的第 39 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
