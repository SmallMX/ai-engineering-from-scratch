...
> 智能体 that can't speak the same language aren't a team. They're strangers shouting into the void.

**类型：** 构建
**语言：** TypeScript
**前置知识：** Phase 14 (智能体 工程), Lesson 16.01 (Why 多智能体)
**时间：** 约 120 minutes

## 学习目标
- Implement MCP tool discovery与invocation so agents can use tools exposed by external servers
- Build an A2A agent card与task endpoint that allows one agent to delegate work to another over HTTP
- Compare MCP (tool access), A2A (agent-to-agent), ACP (enterprise audit),与ANP (decentralized trust)与explain which protocol solves which problem
- Wire multiple protocols together in a single system where agents discover tools via MCP与delegate tasks via A2A

## 中文导读

本课是 Phase 16「多智能体与群体智能」的第 03 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
