# Consensus与Byzantine Fault Tolerance for 智能体

> Classical distributed-systems BFT meets stochastic LLM. In 2025-2026 three research directions emerged：**CP-WBFT** (arXiv:2511.10400) weighs each vote by a confidence probe; **DecentLLMs** (arXiv:2507.14928) goes leaderless with parallel worker proposals与geometric-median aggregation; **WBFT** (arXiv:2505.05103) combines weighted voting with Hierarchical Structure Clustering to split Core与Edge nodes. The honest empirical result from "Can AI 智能体 Agree?" (arXiv:2603.01213) is that even scalar agreement is fragile today：a single deceptive agent can compromise a Mixture-of-智能体. BFT is necessary but not sufficient. This lesson builds a minimal BFT protocol, injects three agent-specific attacks (byzantine lie, sycophantic conformity, correlated-error monoculture),与measures how each consensus variant copes.

**类型：** Learn + Build
**语言：** Python (stdlib)
**前置知识：** Phase 16 · 07 (Society of Mind与Debate), Phase 16 · 13 (Shared 记忆)
**时间：** 约 75 minutes

## 学习目标
- 理解 Consensus与Byzantine Fault Tolerance for 智能体 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 16「多智能体与群体智能」的第 14 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
