...
> 浮点数是一个会漏水的抽象。训练时它一定会咬你，而且你一开始看不见。

**类型：** Build  
**语言：** Python  
**前置知识：** Phase 1，第 04-12 课  
**时间：** 约 75 分钟
...
1. 浮点数有有限精度，会出现 overflow、underflow 和 cancellation。
2. softmax 应先减去最大 logit，避免 `exp` 溢出。
3. log-sum-exp 是很多概率计算的稳定版本。
4. 梯度爆炸和梯度消失都与数值范围有关。
5. 训练中要主动检查 NaN、Inf、dtype、scale 和 clipping。
...
> 英文原文还包含 19 个代码/公式块；中文正文保留关键块，完整可运行代码见本课 `code/` 目录。
