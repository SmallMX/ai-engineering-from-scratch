...
1. 导数衡量函数在某一点的变化率，也就是切线斜率。
2. 偏导数一次只看一个变量，其他变量保持不变；神经网络 loss 对每个权重都有偏导数。
3. gradient 把所有偏导数组成向量，指向函数上升最快的方向；最小化时沿反方向走。
4. gradient descent 用 `w = w - lr * dL/dw` 不断更新参数。
5. Hessian 和 Taylor series 提供二阶曲率信息，但在百万参数网络中直接使用代价太高。
