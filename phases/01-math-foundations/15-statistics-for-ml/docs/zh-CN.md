...
1. 统计量用样本估计总体性质，例如均值、方差和置信区间。
2. hypothesis testing 用 p-value 判断观察到的差异是否可能由随机波动产生。
3. confidence interval 给出估计的不确定范围。
4. A/B test 比较两个方案，但需要注意样本量、功效和多重比较。
5. bootstrap 可以用重复采样估计指标的不确定性。
