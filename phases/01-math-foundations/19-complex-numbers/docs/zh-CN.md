...
> -1 的平方根并不虚幻。它是旋转、频率和半个信号处理世界的钥匙。
...
1. complex number `a + bi` 同时包含实部和虚部。
2. 复数乘法可以表示二维旋转和缩放。
3. Euler formula `e^{iθ} = cos θ + i sin θ` 连接指数、三角函数和旋转。
4. 频域表示、Fourier transform、信号处理和量子计算都大量使用复数。
5. 复数神经网络和谱方法会把 magnitude 与 phase 分开处理。
