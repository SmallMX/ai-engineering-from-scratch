...
## 学习目标
- Implement multi-head cross-attention where the query stream is text与the key/value stream is vision.
...
- Run a forward pass with batched text tokens与a fixed pool of image tokens.

## 中文导读

本课是 Phase 19「毕业项目」的第 61 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
