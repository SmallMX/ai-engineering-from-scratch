# End-to-End Distributed 训练

> Lessons 76 through 80 each built one piece. This is the assembly：a tiny GPT trained across 4 simulated ranks with DDP for gradient sync, ZeRO-1 for optimiser-state sharding,与a sharded checkpoint at the halfway mark. The demo runs 20 steps, self-terminates, prints a loss curve plus a memory profile,与writes a resumable checkpoint.
...
- Print a per-step loss table, a per-rank memory profile,与a checkpoint manifest that resumes byte-equal on the same world size.
- Defend the composition：each piece is independently testable in earlier lessons与this lesson proves they compose.

## 中文导读

本课是 Phase 19「毕业项目」的第 81 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
