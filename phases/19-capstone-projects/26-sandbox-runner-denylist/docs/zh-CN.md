# 毕业项目 Lesson 26：Sandbox Runner with Denylist与Path Jail

> The verification gate decides whether a tool call should run. The sandbox decides what happens when it does. This lesson ships a subprocess runner that refuses dangerous executables, refuses dangerous argv shapes, jails every file path to a project root, truncates oversized output,与kills runaway processes on a wall-clock timeout. It is the second of two layers that sit between the model与the operating system.

**类型：** 构建
**语言：** Python (stdlib)
**前置知识：** Phase 19 · 25 (verification gates与observation budget), Phase 14 · 33 (instructions as constraints), Phase 14 · 38 (verification gates)
**时间：** 约 90 minutes

## 学习目标
- Build a `Sandbox` class wrapping `subprocess.run` with timeout, capture,与truncation.
- Refuse a command by name against a denylist与by structure against an argv inspector.
- Refuse any path argument that resolves outside a declared project root.
- Refuse shell metacharacters when shell mode is off.
- Return a structured `SandboxResult` that downstream observability与the eval harness can ingest.

## 中文导读

本课是 Phase 19「毕业项目」的第 26 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
