...
> Ids are integers. The model wants vectors. Two lookup tables sit between them,与the choice of the positional one shapes what the model can learn.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 04 lessons, Phase 07 transformer lessons, Lessons 30与31 of this phase
**时间：** 约 90 minutes
...
- Compose token与positional embeddings into a single input for a transformer block.
- Contrast learned与sinusoidal embeddings on length generalization与parameter count.

## 中文导读

本课是 Phase 19「毕业项目」的第 32 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
