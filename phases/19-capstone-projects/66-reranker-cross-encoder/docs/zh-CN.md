...
> A bi-encoder embeds query与document independently. A cross-encoder concatenates them与reads both at once. The cross-encoder is the smartest reader与the slowest. Used as a second stage on the bi-encoder's top-k, it pays for itself.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 11 lesson 06 (RAG), Phase 11 lesson 07 (advanced RAG); Phase 19 Track B foundations (lessons 20-29); Phase 19 lesson 65 (hybrid retrieval feeding this stage)
**时间：** 约 90 minutes

## 学习目标
- Distinguish a bi-encoder retriever from a cross-encoder reranker by their input shape, parameter count,与per-query cost.
- Implement a small cross-encoder from scratch as a transformer block that consumes a packed (query, document) sequence与emits a single relevance scalar.
...
- Measure the latency-vs-quality trade-off on a small fixture corpus与pick the right N for a given latency budget.

## 中文导读

本课是 Phase 19「毕业项目」的第 66 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
