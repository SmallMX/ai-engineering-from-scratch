# 毕业项目 07：End-to-End 微调 Pipeline (数据 to SFT to DPO to Serve)

> An 8B model trained on your own data, DPO-aligned on your own preferences, quantized, speculative-decoded,与served at measurable $/1M tokens. The 2026 open stack is Axolotl v0.8, TRL 0.15, Unsloth for iteration, GPTQ/AWQ/GGUF for quantization, vLLM 0.7 with EAGLE-3 for serving. The capstone is to run the whole pipeline reproducibly：YAML in, served endpoint out —与publish a model card under the 2026 Model Openness 框架.

**类型：** 毕业项目
**语言：** Python (pipeline), YAML (configs), Bash (scripts)
**前置知识：** Phase 2 (ML), Phase 3 (DL), Phase 7 (transformers), Phase 10 (LLM from scratch), Phase 11 (LLM engineering), Phase 17 (infrastructure), Phase 18 (safety)
**时间：** 35 hours

## 学习目标
- 理解 毕业项目 07：End-to-End 微调 Pipeline (数据 to SFT to DPO to Serve) 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 19「毕业项目」的第 07 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
