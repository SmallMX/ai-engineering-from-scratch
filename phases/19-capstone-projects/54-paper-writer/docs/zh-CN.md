...
> A LaTeX skeleton is a contract between the researcher与the typesetter. If the contract is broken the document does not compile,与the failure is loud. Build the skeleton first, then fill it.
...
- Generate a LaTeX skeleton that declares its abstract, sections, figure slots,与bibliography keys before any prose is written.
- Inject figures from experiment outputs (paths与captions) into the skeleton through a deterministic slot mechanism.
...
- Emit a single `paper.tex` plus a `references.bib` plus a manifest that lists every figure referenced与every citation used.

## 中文导读

本课是 Phase 19「毕业项目」的第 54 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
