# 数据 Parallel DDP 从零实现

> DistributedDataParallel is a hook on top of allreduce. Wrap a model, broadcast the initial parameters from rank 0 so every rank starts identical, install a backward hook on every parameter that issues an allreduce of the gradient,与the rest is gradient descent. The whole pattern is 200 lines.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 19 Track C lessons 42-49
**时间：** 约 90 min

## 学习目标
- Wire a `DistributedDataParallel`-shaped wrapper that broadcasts initial parameters与allreduces gradients after backward.
- Spawn N CPU ranks with `torch.multiprocessing.spawn` over the gloo backend with file-based rendezvous.
- Prove gradient-sync correctness by training the same model on the same data sequentially与showing per-step parameter equivalence.
- Defend the use of buckets (gradient fusion)与overlap (comm during backward) as the two changes that turn a working DDP into a production DDP.

## 中文导读

本课是 Phase 19「毕业项目」的第 77 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
