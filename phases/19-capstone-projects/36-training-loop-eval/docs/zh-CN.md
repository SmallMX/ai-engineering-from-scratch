...
> A loop that does not measure is a loop that lies. This lesson builds the training loop that drives the GPT model：AdamW with weight decay split, a warmup plus cosine learning rate schedule, a `calc_loss_batch` helper, an `evaluate_model` pass on held out data, a `generate_and_print_sample` qualitative probe every K steps,与a JSONL log of losses you can plot after. The same skeleton trains every decoder LLM you will ever build.
...
## 学习目标
- Build a training loop that computes cross entropy loss with the correct input与target alignment for next token prediction.
- Configure AdamW with weight decay applied to weight tensors与not to LayerNorm or bias tensors.
- Implement a learning rate schedule with linear warmup与cosine decay,与read the resulting LR over time.
...
- Persist per step loss to JSONL so you can reload, plot,与ship the training log as a deliverable.

## 中文导读

本课是 Phase 19「毕业项目」的第 36 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
