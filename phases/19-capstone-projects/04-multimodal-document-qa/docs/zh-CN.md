# 毕业项目 04：多模态 Document QA (视觉-First PDF, Tables, Charts)

> The 2026 document-QA frontier moved away from OCR-then-text与toward vision-first late interaction. ColPali, ColQwen2.5,与ColQwen3-omni treat each PDF page as an image, embed it with multi-vector late interaction,与let the query attend to patches directly. On financial 10-Ks, scientific papers,与handwritten notes this pattern beats OCR-first by a large margin. Build the pipeline end to end on 10k pages与publish the side-by-side against OCR-then-text.
...
## 学习目标
- 理解 毕业项目 04：多模态 Document QA (视觉-First PDF, Tables, Charts) 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 19「毕业项目」的第 04 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
