...
- Compute scaled dot-product attention with the correct normalization与dtype handling.
- Apply a causal mask that prevents a position from attending to future positions.
- Inspect per-head attention weights for a fixed input与reason about what each head looks at.
- Train a small attention block on a toy task与watch the loss fall as the heads specialize.

## 中文导读

本课是 Phase 19「毕业项目」的第 33 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
