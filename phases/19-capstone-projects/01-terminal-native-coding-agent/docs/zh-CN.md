# 毕业项目 01：Terminal-Native Coding 智能体

> By 2026 the shape of a coding agent is settled. A TUI harness, a stateful plan, a sandboxed tool surface, a loop that plans, acts, observes, recovers. Claude Code, Cursor 3,与OpenCode all look the same from 50 feet. This capstone asks you to build one end to end：CLI in, pull request out —与measure it against mini-swe-agent与Live-SWE-agent on SWE-bench Pro. You will learn why the hard part is not the model call but the tool loop, the sandbox,与the cost ceiling on a 50-turn run.

**类型：** 毕业项目
**语言：** TypeScript / Bun (harness), Python (eval scripts)
**前置知识：** Phase 11 (LLM engineering), Phase 13 (tools与protocols), Phase 14 (agents), Phase 15 (autonomous systems), Phase 17 (infrastructure)
**时间：** 35 hours

## 学习目标
- 理解 毕业项目 01：Terminal-Native Coding 智能体 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 19「毕业项目」的第 01 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
