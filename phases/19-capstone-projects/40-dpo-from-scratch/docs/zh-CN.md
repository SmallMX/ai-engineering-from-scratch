# 毕业项目 Lesson 40：Direct Preference Optimization 从零实现

> 奖励 models与PPO are the classical RLHF stack. DPO collapses that stack into a single supervised loss that fits a policy directly against preference pairs. This lesson derives the DPO loss from the reward-difference identity, ships a working reference model plus policy model, computes per-token log-probabilities,与trains a tiny transformer on a preference fixture of chosen与rejected completions. Tests pin the loss math与the gradient direction so you know the implementation matches the paper.
...
## 学习目标
- Derive the DPO loss as a sigmoid over a scaled log-ratio difference与connect it to the implicit reward.
- Build a reference model + policy model pair with a frozen reference与a trainable policy.
- Compute sequence-level log-probabilities under both models, masking prompt tokens.
- Train the policy on `(prompt, chosen, rejected)` triples与watch the chosen log-prob rise relative to rejected.
- Pin behaviour with tests on the loss math, the gradient sign,与the reference invariance.

## 中文导读

本课是 Phase 19「毕业项目」的第 40 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
