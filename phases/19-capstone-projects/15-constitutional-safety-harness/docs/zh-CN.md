# 毕业项目 15：Constitutional 安全 Harness + Red-Team Range

> Anthropic's Constitutional 分类器, Meta's Llama Guard 4, Google's ShieldGemma-2, NVIDIA's Nemotron 3 Content 安全,与X-Guard for multilingual coverage defined the 2026 safety-classifier stack. garak, PyRIT, NVIDIA Aegis,与promptfoo became the standard adversarial evaluation tools. NeMo Guardrails v0.12 ties them into a production pipeline. This capstone wires all of it together：a layered safety harness around a target app, an autonomous red-team agent running 6+ attack families,与a constitutional self-critique run that produces a measurable harmlessness delta.

**类型：** 毕业项目
**语言：** Python (safety pipeline, red team), YAML (policy configs)
**前置知识：** Phase 10 (LLM from scratch), Phase 11 (LLM engineering), Phase 13 (tools), Phase 14 (agents), Phase 18 (ethics, safety, alignment)
**时间：** 25 hours

## 学习目标
- 理解 毕业项目 15：Constitutional 安全 Harness + Red-Team Range 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 19「毕业项目」的第 15 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
