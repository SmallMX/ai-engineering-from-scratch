...
- Implement a deterministic merge table与apply it to fresh text to produce a stream of subword ids.
- Round-trip arbitrary UTF-8 input to ids与back without information loss.
- Reserve与protect special tokens (`<|endoftext|>`, `<|pad|>`) so they survive training与decoding.
- Reason about why a byte-level alphabet is the right floor for a general-purpose tokenizer.

## 中文导读

本课是 Phase 19「毕业项目」的第 30 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
