# Perplexity与Calibration

> If your model says 90 percent confident on a thousand answers与gets six hundred right, it is not well calibrated. Calibration is half of trustworthy eval. The other half is perplexity, which tells you whether the model thinks the held-out text is plausible at all.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 19 Track B foundations, lessons 70与71
**时间：** 约 90 min

## 学习目标
- 理解 Perplexity与Calibration 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 19「毕业项目」的第 73 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
