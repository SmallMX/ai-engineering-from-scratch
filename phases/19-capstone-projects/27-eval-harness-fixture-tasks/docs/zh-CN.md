# 毕业项目 Lesson 27：评估 Harness with Fixture Tasks

> A coding agent is only as good as the suite of tasks you measure it against. This lesson builds an evaluation harness that takes a folder of fixture tasks, runs each through a candidate agent, scores pass or fail through a deterministic verifier,与aggregates the results into pass@1, pass@k, mean latency,与mean cost. The harness is the source of truth that lets you tell a regression from a refactor.

**类型：** 构建
**语言：** Python (stdlib)
**前置知识：** Phase 19 · 25 (verification gates), Phase 19 · 26 (sandbox runner), Phase 14 · 30 (eval-driven agent development), Phase 14 · 19 (SWE-bench与GAIA benchmarks)
**时间：** 约 90 minutes

## 学习目标
- Define a fixture task as a triple of goal, setup,与verifier.
- Score multiple sample runs per task与compute pass@1与pass@k.
- Aggregate latency与cost into mean与95th-percentile metrics.
...
本课是 Phase 19「毕业项目」的第 27 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
