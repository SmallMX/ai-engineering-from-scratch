...
> If you cannot grade your retrieval与your answer at the same time, you cannot ship the system. The two are not the same metric与the same prompt fails on different axes.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 11 lessons 06 (RAG), 10 (evaluation); Phase 19 Track B foundations (lessons 20-29); Phase 19 lessons 64, 65, 66, 67
**时间：** 约 90 minutes

## 学习目标
- Compute four retrieval metrics from gold qrels：precision@k, recall@k, MRR (mean reciprocal rank),与nDCG@k.
- Compute two answer-grade metrics：faithfulness (every claim grounded in retrieved context)与answer relevance (the answer addresses the question).
...
本课是 Phase 19「毕业项目」的第 68 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
