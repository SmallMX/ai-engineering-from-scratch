# Hybrid 检索 with BM25与Dense 嵌入

> Lexical与semantic retrieval fail on opposite query distributions. Hybrid retrieval with reciprocal rank fusion does not interpolate, it votes -与the vote wins on every query class.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 11 lessons 04 (embeddings), 06 (RAG); Phase 19 Track B foundations (lessons 20-29); Phase 19 lesson 64 (chunking strategies)
**时间：** 约 90 minutes

## 学习目标
- Implement BM25 from scratch from the Robertson与Sparck Jones formulation, with field weighting, document length normalization,与tunable k1与b.
- Build a dense retriever on top of a deterministic mock embedding so the loop runs offline.
- Implement reciprocal rank fusion exactly as Cormack, Clarke,与Buettcher published it in 2009,与explain why it dominates score-weighted interpolation.
- Tune the RRF k constant与the per-modality weights与read the trade-offs on a small fixture corpus.

## 中文导读

本课是 Phase 19「毕业项目」的第 65 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
