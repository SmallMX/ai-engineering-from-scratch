...
## 学习目标
- Implement a pre-LN transformer block with multi-head self-attention与a feed-forward sub-layer.
- Stack 12 blocks with 12 heads to form a ViT-Base encoder.
- Wire the patch front end from lesson 58 into the encoder与run a forward pass.
- Verify that the CLS token aggregates information from every patch.

## 中文导读

本课是 Phase 19「毕业项目」的第 59 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
