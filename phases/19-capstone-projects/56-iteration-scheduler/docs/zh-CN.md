...
> A research loop without a scheduler is a queue with delusions. The scheduler is where the loop decides what to stop exploring,与that decision is the whole game.
...
- Fan out finished results to a paper-write stage与a re-queue stage so a high-yield branch spawns follow-up hypotheses.
- Surface a per-iteration trace with branch scores, slot occupancy,与pruning decisions.

## 中文导读

本课是 Phase 19「毕业项目」的第 56 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
