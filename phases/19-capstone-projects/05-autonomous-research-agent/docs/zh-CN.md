# 毕业项目 05：自主 Research 智能体 (AI-Scientist Class)

> Sakana's AI-Scientist-v2 published full papers. 智能体 Laboratory ran the experiments. Allen AI shared traces. The 2026 shape is plan-execute-verify tree search over experiments, budgeted cost, sandboxed code execution, a vision-feedback LaTeX writer,与an automated NeurIPS-style reviewer ensemble. The capstone is to build one, run it end to end within $30 per paper,与survive the sandbox-escape red team that Sakana documented.

**类型：** 毕业项目
**语言：** Python (agent + sandbox), LaTeX (output)
**前置知识：** Phase 2 (ML), Phase 3 (deep learning), Phase 7 (transformers), Phase 10 (LLM from scratch), Phase 14 (agents), Phase 15 (autonomous), Phase 16 (multi-agent), Phase 18 (safety)
**时间：** 40 hours

## 学习目标
- 理解 毕业项目 05：自主 Research 智能体 (AI-Scientist Class) 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 19「毕业项目」的第 05 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
