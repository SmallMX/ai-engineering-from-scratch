# 毕业项目 Lesson 41：Full 评估 Pipeline

> 训练 is the part you can monitor with loss curves. 评估 is the part you have to design. This lesson builds a unified eval pipeline that takes any trained language model, runs four heterogeneous evals on it, aggregates the results into a per-task report,与ships a local mock LLM-as-judge so the loop runs without a network. The four evals cover the dimensions every shipping model needs：language modelling (perplexity), short-form correctness (exact-match), open-form similarity (token F1),与qualitative scoring (judge).
...
- Compute token-level F1 between predicted与reference strings with normalisation.
...
本课是 Phase 19「毕业项目」的第 41 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
