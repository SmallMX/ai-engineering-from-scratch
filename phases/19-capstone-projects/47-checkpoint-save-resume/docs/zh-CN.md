# Checkpoint Save与Resume

> Train interrupts kill runs; checkpoints let them continue. Save model, optimizer, scheduler, loss history, step counter,与RNG state, atomically, so a kill at any moment leaves a valid file on disk.
...
- Restore the RNG state for Python, NumPy,与PyTorch so the post-resume loss matches the uninterrupted baseline.
- Build a sharded checkpoint layout for models that no longer fit in a single file, with hash-verified shards与a JSON index.

## 中文导读

本课是 Phase 19「毕业项目」的第 47 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
