# 视觉-Language Pretraining

> The encoder, projection,与decoder are wired. Now train them together. Two objectives drive learning：a contrastive image-text loss (InfoNCE) that pulls matching pairs together in the joint embedding space,与a language modeling loss that asks the decoder to caption each image. Combined, they teach the network both to find the right image for a caption与to write a caption for the image.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 19 lessons 30-37 (Track B foundations)
**时间：** 约 90 minutes
...
- Run a 50-step demo training loop与observe both losses decreasing.

## 中文导读

本课是 Phase 19「毕业项目」的第 62 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
