...
## 学习目标
- Read a safetensors file with the `safetensors` Python library与inspect the tensor names与shapes.
- Map each pretrained parameter name onto a parameter inside the lesson 35 GPT model.
- Handle the two name conventions that differ between published GPT-2 weights与the model in this track：`wte/wpe/h.N.attn.c_attn/c_proj`与`mlp.c_fc/c_proj` versus the locally named `tok_embed/pos_embed/blocks.N.attn.qkv/out_proj`与`mlp.fc1/fc2`.
- Detect与refuse a shape mismatch with a clear error before any weight assignment happens.
- Generate a short continuation with the loaded weights与confirm the tokens come from the loaded distribution, not the randomly initialized one.

## 中文导读

本课是 Phase 19「毕业项目」的第 37 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
