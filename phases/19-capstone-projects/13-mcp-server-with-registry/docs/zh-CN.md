# 毕业项目 13：MCP Server with Registry与治理

> The 模型上下文协议 stopped being the future与became the default tool-use spec in 2026. Anthropic, OpenAI, Google,与every major IDE ship MCP clients. Pinterest published its internal ecosystem of MCP servers. The AAIF Registry formalized capability metadata at `.well-known`. AWS ECS published the reference stateless deployment. Block's goose-agent put the same protocol inside a hosted assistant. The 2026 production shape is：StreamableHTTP transport, OAuth 2.1 scopes, OPA policy gating,与a registry that lets platform teams discover, validate,与enable servers. Build that end to end.

**类型：** 毕业项目
**语言：** Python (server, via FastMCP) or TypeScript (@modelcontextprotocol/sdk), Go (registry service)
**前置知识：** Phase 11 (LLM engineering), Phase 13 (tools与MCP), Phase 14 (agents), Phase 17 (infrastructure), Phase 18 (safety)
**时间：** 25 hours

## 学习目标
- 理解 毕业项目 13：MCP Server with Registry与治理 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 19「毕业项目」的第 13 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
