...
> A tool the agent cannot validate is a tool the agent cannot call. Build the registry与the schema checker before you build the tools.
...
## 学习目标
- Hold a typed registry of tool name → schema → handler that the dispatcher can ask once与trust afterwards.
...
本课是 Phase 19「毕业项目」的第 21 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
