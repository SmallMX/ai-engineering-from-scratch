# Query Rewriting：HyDE, Multi-Query,与Decomposition
...
**类型：** 构建
**语言：** Python
**前置知识：** Phase 11 lessons 04 (embeddings), 06 (RAG); Phase 19 Track B foundations (lessons 20-29); Phase 19 lessons 64与65
**时间：** 约 90 minutes

## 学习目标
- Implement Hypothetical Document 嵌入 (HyDE)：generate a fake answer, embed it, retrieve against that vector instead of the query vector.
...
- Compare the three rewriters head to head on a fixture与explain when each strategy wins.
- Wire a mock LLM that produces deterministic, on-fixture outputs so the rewriter loop runs offline.

## 中文导读

本课是 Phase 19「毕业项目」的第 67 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
