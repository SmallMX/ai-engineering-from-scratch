...
> 训练 is half the loop. The other half is measurement. This lesson builds three evaluation surfaces from primitives：image-caption retrieval reported as R@1, R@5, R@10; visual question answering reported as exact match accuracy;与image captioning reported as BLEU-4. Each metric is a function over the model's outputs与a synthetic eval suite that runs in seconds.

**类型：** 构建
**语言：** Python
**前置知识：** Phase 19 lessons 58-62 (Track E foundations：encoder, transformer, projection, cross-attention fusion, pretraining)
**时间：** 约 90 minutes

## 学习目标
- Compute Recall@K from a similarity matrix between image与caption embeddings.
- Compute exact-match VQA accuracy from a model that maps (image, question) pairs to a fixed answer vocabulary.
- Compute BLEU-4 from generated与reference token sequences without any external library.
- Run all three evals against a synthetic suite built on top of the trained model from lesson 62.

## 中文导读

本课是 Phase 19「毕业项目」的第 63 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
