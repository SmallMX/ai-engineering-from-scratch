# Tokenized 数据集 with Sliding Window
...
- Build a PyTorch 数据集 that returns input与target tensors for next-token prediction.
- Wrap the dataset in a DataLoader with a deterministic shuffle seeded per epoch.
- Reason about the trade-off between stride, redundancy,与effective dataset size.

## 中文导读

本课是 Phase 19「毕业项目」的第 31 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
