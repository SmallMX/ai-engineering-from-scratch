...
- Implement ten lifecycle hook topics that operators wire policy, telemetry,与guardrails into.
- Define two pull points where the loop yields control back to the caller与resumes on a fresh input.
- Enforce per-session budgets (turns, tool calls, wall-clock) without leaking partial state on exceeding.
- Emit a typed stream of eleven event types so downstream UIs与tracers can subscribe without inspecting the loop directly.

## 中文导读

本课是 Phase 19「毕业项目」的第 20 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
