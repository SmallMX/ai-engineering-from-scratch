# Long-Running Background 智能体：Durable Execution

> 生产 long-horizon agents do not run in `while True`. Every LLM call becomes an activity with checkpoint, retry,与replay. Temporal's OpenAI 智能体 SDK integration went GA March 2026. Claude Code Routines (Anthropic) runs scheduled Claude Code invocations without a persistent local process. Sessions pause on human-input, survive deploys,与resume from the latest checkpoint keyed by `thread_id`. Behind the new ergonomics sits an old pattern：workflow orchestration：with one new input：LLM calls as non-deterministic activities that must be deterministically replayed on recovery.
...
## 学习目标
- 理解 Long-Running Background 智能体：Durable Execution 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 15「自主系统」的第 12 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
