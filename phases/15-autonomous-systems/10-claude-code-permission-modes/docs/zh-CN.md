# Claude Code as an 自主 智能体：Permission Modes与Auto Mode

> Claude Code exposes seven permission modes. "plan" asks before every action, "default" asks only for risky ones, "acceptEdits" auto-approves file writes but still confirms shell execution,与"bypassPermissions" approves everything. Auto Mode (March 24, 2026) replaces per-action approval with a two-stage parallel safety classifier：a single-token fast check runs on every action; flagged actions kick off a chain-of-thought deep review. Action budgets are enforced via `max_turns`与`max_budget_usd`. Auto Mode shipped as a research preview：Anthropic has stated explicitly that the classifier is not sufficient alone.

**类型：** 学习
**语言：** Python (stdlib, two-stage classifier simulator)
**前置知识：** Phase 15 · 01 (Long-horizon agents), Phase 15 · 09 (Coding-agent landscape)
**时间：** 约 45 minutes

## 学习目标
- 理解 Claude Code as an 自主 智能体：Permission Modes与Auto Mode 在本阶段课程中的作用。
- 把核心概念映射到可运行代码、测验和课程产物。
- 保留英文术语、命令、路径和 API 名称，方便和原文对照。

## 中文导读

本课是 Phase 15「自主系统」的第 10 课。学习时建议先读这一份中文导读，确认本课要解决的问题、关键术语和可运行产物，再回到英文原文核对细节。
